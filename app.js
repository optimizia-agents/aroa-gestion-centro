...
function exportCurrentWorkbook(){
  ...
  const procedures=[['N.º de procedimiento','N.º de EMOC','Procedimiento / documento','Versión','Autor','Estado','Notas / siguiente acción','Enlace'],...procedureRows.map(r=>[r.number||'',r.emocNumber||'',r.name||'',r.version||'',r.author||'',r.status||'Pendiente',r.notes||'',r.url||''])];
...
function renderProcedures(){
 ...
 const rows=procedureRows.filter(row=>(filter==='all'||String(row.status||'Pendiente')===filter)&&[row.number,row.emocNumber,row.name,row.version,row.author,row.status,row.notes].join(' ').toLocaleLowerCase('es').includes(query)).sort((a,b)=>String(a.name||'').localeCompare(String(b.name||''),'es',{sensitivity:'base',numeric:true}));
