...
function renderEconomicAnalysis(){
 ...
 const yearEntries=[...yearTotals.entries()].sort((a,b)=>String(a[0]).localeCompare(String(b[0]))),yearNode=$('analysis-year-chart');
 if(yearNode){if(!yearEntries.length)yearNode.innerHTML='<p class="empty">No hay años para este filtro.</p>';else{const maxYear=Math.max(...yearEntries.map(([,value])=>value),1);yearNode.innerHTML=`<div class="analysis-columns">${yearEntries.map(([label,value])=>`<div class="analysis-column"><strong>${analysisCurrency(value)}</strong><div class="analysis-column-track"><span style="height:${Math.max(5,(value/maxYear)*100)}%"></span></div><b>${htmlEscape(label)}</b></div>`).join('')}</div>`}}
