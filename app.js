...
function renderTemplates(){
 ...
 body.innerHTML=rows.length?rows.map(row=>{const index=templateRows.indexOf(row);return `<article class="template-card" role="listitem"><div class="template-card-icon" aria-hidden="true">▤</div><div class="template-card-content"><h4>${htmlEscape(row.name||'Sin nombre')}</h4><div class="template-card-actions">${row.url?`<a class="primary template-open-link" href="${htmlEscape(row.url)}" target="_blank" rel="noopener noreferrer">ABRIR EN SHAREPOINT</a>`:'<span class="muted">Enlace pendiente</span>'}<button class="edit-btn" type="button" onclick="openTemplateEditor(${index})">EDITAR</button></div></div></article>`}).join(''):'<div class="empty template-empty">No hay plantillas que coincidan con la búsqueda.</div>';
...
function renderProcedures(){
 ...
 body.innerHTML=rows.length?rows.map(row=>{const index=procedureRows.indexOf(row),status=row.status||'Pendiente';return `<tr><td><span class="procedure-number">${htmlEscape(row.number||'Sin indicar')}</span><span class="procedure-name">${htmlEscape(row.name||'Sin nombre')}</span></td><td>${htmlEscape(row.version||'Sin indicar')}</td><td>${htmlEscape(row.author||'Sin indicar')}</td><td><span class="status ${procedureStatusClass(status)}">${htmlEscape(status)}</span></td><td>${htmlEscape(row.notes||'Sin comentarios')}</td><td>${row.url?`<a class="center-evidence-link" href="${htmlEscape(row.url)}" target="_blank" rel="noopener noreferrer" aria-label="Abrir procedimiento en SharePoint" title="Abrir procedimiento en SharePoint">▤</a>`:'<span class="center-evidence-link evidence-missing" role="img" aria-label="Enlace pendiente" title="Enlace pendiente">▤</span>'}</td><td><button class="edit-btn" type="button" onclick="openProcedureEditor(${index})">EDITAR</button></td></tr>`}).join(''):'<tr><td colspan="7" class="empty">No hay procedimientos que coincidan con los filtros.</td></tr>';
 $('procedure-count').textContent=`${rows.length} procedimientos`;
}
function openProcedureEditor(index=-1){procedureEditorIndex=index;const row=index<0?{}:procedureRows[index]||{};$('procedure-edit-number').value=row.number||'';$('procedure-edit-name').value=row.name||'';$('procedure-edit-version').value=row.version||'';$('procedure-edit-author').value=row.author||'';$('procedure-edit-status').value=row.status||'Pendiente';$('procedure-edit-url').value=row.url||'';$('procedure-edit-notes').value=row.notes||'';$('procedure-editor-title').textContent=index<0?'Nuevo procedimiento':'Editar procedimiento';$('delete-procedure-editor').hidden=index<0;$('procedure-editor').classList.add('open');$('procedure-editor').setAttribute('aria-hidden','false');$('procedure-edit-name').focus()}
...
async function saveProcedureEditor(){
 ...
 if(url){try{if(new URL(url).protocol!=='https:')throw new Error()}catch{showSyncToast('Añade un enlace HTTPS válido de SharePoint.');return}}
 const next=structuredClone(procedureRows),item={id:procedureEditorIndex<0?crypto.randomUUID():(next[procedureEditorIndex]?.id||crypto.randomUUID()),number:$('procedure-edit-number').value.trim(),name,version:$('procedure-edit-version').value.trim(),author:$('procedure-edit-author').value.trim(),status:$('procedure-edit-status').value,notes:$('procedure-edit-notes').value.trim(),url};if(procedureEditorIndex<0)next.push(item);else next[procedureEditorIndex]=item;
...
function applyConfirmedDocument(doc){
 ...
 appDocument.procedures=(storedProcedures.length===0?DEFAULT_PROCEDURE_ROWS:storedProcedures).map(row=>{const saved=procedureById.get(row.id)||row;return {id:saved.id||crypto.randomUUID(),number:saved.number||row.number||'',name:saved.name||row.name||'',version:saved.version||row.version||'',author:saved.author||row.author||'',status:saved.status||row.status||'Pendiente',notes:saved.notes||row.notes||'',url:saved.url||row.url||''}});
