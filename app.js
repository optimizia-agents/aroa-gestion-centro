...
function renderImprovements(){
 ...
 const providers=[...new Set(improvementRows.map(row=>String(row.provider||'').trim()).filter(Boolean))].sort((a,b)=>a.localeCompare(b,'es',{sensitivity:'base'}));const providerSelect=$('improvement-provider-filter');if(providerSelect){const current=providerSelect.value;providerSelect.innerHTML='<option value="all">Todos los proveedores</option>'+providers.map(item=>`<option value="${htmlEscape(item)}">${htmlEscape(item)}</option>`).join('');providerSelect.value=providers.includes(current)?current:'all'}
