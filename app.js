...
function renderProcedures(){
 ...
 body.innerHTML=rows.length?rows.map(row=>{const index=procedureRows.indexOf(row),status=row.status||'Pendiente';return `<tr><td>${htmlEscape(row.number||'Sin indicar')}</td><td>${htmlEscape(row.name||'Sin nombre')}</td><td>${htmlEscape(row.version||'Sin indicar')}</td><td>${htmlEscape(row.author||'Sin indicar')}</td><td><span class="status ${procedureStatusClass(status)}">${htmlEscape(status)}</span></td><td>${htmlEscape(row.notes||'Sin comentarios')}</td><td>${row.url?`<a class="procedure-open-link" href="${htmlEscape(row.url)}" target="_blank" rel="noopener noreferrer">ABRIR EN SHAREPOINT</a>`:'<span class="muted">Enlace pendiente</span>'}</td><td><button class="edit-btn" type="button" onclick="openProcedureEditor(${index})">EDITAR</button></td></tr>`}).join(''):'<tr><td colspan="8" class="empty">No hay procedimientos que coincidan con los filtros.</td></tr>';
