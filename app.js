...
function ensureSharePointField(){const grid=$('center-editor')?.querySelector('.editor-grid');if(!grid||$('edit-sharepoint-url'))return;const label=document.createElement('label');label.className='editor-wide';label.innerHTML='Enlace al archivo de SharePoint<input id="edit-sharepoint-url" type="text" inputmode="url" placeholder="https://...sharepoint.com/..." autocomplete="off" autocapitalize="none" spellcheck="false"><small class="field-help">El archivo seguirá alojado en SharePoint. Aquí solo guardamos su enlace.</small><a id="edit-sharepoint-open" class="sharepoint-link" href="#" target="_blank" rel="noopener" hidden>Abrir archivo en SharePoint</a>';grid.appendChild(label);$('edit-sharepoint-url').addEventListener('input',()=>{const value=$('edit-sharepoint-url').value.trim();const link=$('edit-sharepoint-open');if(link){link.href=value||'#';link.hidden=!/^https:\/\//i.test(value)}})}
