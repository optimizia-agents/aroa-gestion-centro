...
function renderImprovements(){
 const query=String($('improvement-search')?.value||'').trim().toLocaleLowerCase('es'),filter=$('improvement-status-filter')?.value||'all';
 const rows=improvementRows.filter(row=>(filter==='all'||String(row.status||'Necesidad detectada')===filter)&&[row.name,row.category,row.date,row.provider,row.requestNumber,row.request,row.orderNumber,row.amount,row.status,row.justification,row.action,row.result].join(' ').toLocaleLowerCase('es').includes(query));
