...
function purchaseProviderContactGroup(provider){const raw=canonicalProvider(provider);if(!raw)return'';const key=Object.keys(providerContactsByName||{}).find(name=>name!==INTERNAL_CONTACTS_KEY&&canonicalProvider(name).toLocaleLowerCase('es')===raw.toLocaleLowerCase('es'));return key||raw}
...
async function saveProviderEditor(){const name=$('provider-edit-name').value.trim().replace(/\s+/g,' ');if(!name){showSyncToast('Escribe el nombre del proveedor');return}if(!firebaseUser||!appRevision){showSyncToast('Actualiza los datos de Firebase antes de guardar');return}const next=providerDirectoryRows().filter(row=>!providerEditorKey||providerDirectoryKey(row.name)!==providerEditorKey);next.push({name,supplierNumber:$('provider-edit-supplier').value.trim(),description:$('provider-edit-description').value.trim()});const payload=structuredClone(appDocument);payload.providerDirectory=next;payload.lists=mergedKurroLists();payload.lists.provider=[...new Set([...(payload.lists.provider||[]),name])];payload.updated=new Date().toISOString();payload.lastMutation=crypto.randomUUID();try{const confirmed=await writeDocument('appState','main',payload,appRevision);applyConfirmedDocument(confirmed);closeProviderEditor();showSyncToast('Proveedor guardado')}catch(error){showSyncToast('No se ha podido guardar el proveedor')}}
