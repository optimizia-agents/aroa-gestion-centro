...
const SAP_IMPORT_ROWS = [
 {id:'sap-4506816352',name:'Mantenimiento trimestral contraincendios del Sales Center Fuenlabrada',category:'Reparación / mantenimiento',status:'Finalizado',date:'02/10/2025',provider:'CYRASA SEGURIDAD SL',orderNumber:'4506816352',amount:'216,86 €',observations:'Importado de GASTOS 1316.xlsx; fecha del movimiento SAP.'},
 {id:'sap-4506795496',name:'Licencia CAE Plus 2025',category:'Servicio',status:'Finalizado',date:'17/07/2025',provider:'E COORDINA GESTION DOCUMENTAL S L',orderNumber:'4506795496',amount:'-0,01 €',observations:'Importado de GASTOS 1316.xlsx; fecha del movimiento SAP.'},
 {id:'sap-4506861372',name:'Jaula de protección para cesta',category:'Seguridad / cumplimiento',status:'Finalizado',date:'02/11/2025',provider:'ESTRUCTURAS METALICAS SERRANO E',orderNumber:'4506861372',amount:'0,00 €',observations:'Importado de GASTOS 1316.xlsx; fecha del movimiento SAP.'},
 {id:'sap-4506645062',name:'Limpieza del Sales Center Fuenlabrada 2025',category:'Servicio',status:'Finalizado',date:'24/02/2025',provider:'ISS FACILITY SERVICES SA',orderNumber:'4506645062',amount:'2970,27 €',observations:'Importado de GASTOS 1316.xlsx; fecha del movimiento SAP.'},
 {id:'sap-4400754144',name:'Bolígrafos y papel de oficina',category:'Suministro / consumible',status:'Finalizado',date:'30/09/2025',provider:'KALAMAZOO PRODUCTOS DE OFICINA SL',orderNumber:'4400754144',amount:'21,13 €',observations:'Importado de GASTOS 1316.xlsx; fecha del movimiento SAP.'},
 {id:'sap-4400760543',name:'Agendas y pilas',category:'Suministro / consumible',status:'Finalizado',date:'06/11/2025',provider:'KALAMAZOO PRODUCTOS DE OFICINA SL',orderNumber:'4400760543',amount:'34,87 €',observations:'Importado de GASTOS 1316.xlsx; fecha del movimiento SAP.'},
 {id:'sap-4400789190',name:'Toallas de papel y bandejas de escritorio',category:'Suministro / consumible',status:'Finalizado',date:'08/05/2026',provider:'KALAMAZOO PRODUCTOS DE OFICINA SL',orderNumber:'4400789190',amount:'88,97 €',observations:'Importado de GASTOS 1316.xlsx; fecha del movimiento SAP.'},
 {id:'sap-4506670015',name:'Mantenimiento de carretilla LINDE E25 · año 1',category:'Reparación / mantenimiento',status:'Finalizado',date:'09/05/2025',provider:'LINDE MATERIAL HANDLING IBERICA, SA',orderNumber:'4506670015',amount:'240,00 €',observations:'Importado de GASTOS 1316.xlsx; fecha del movimiento SAP.'},
 {id:'sap-4506104466',name:'Mantenimiento de carretilla LINDE E25 · año 1',category:'Reparación / mantenimiento',status:'Finalizado',date:'05/12/2025',provider:'LINDE MATERIAL HANDLING IBERICA, SA',orderNumber:'4506104466',amount:'30,00 €',observations:'Importado de GASTOS 1316.xlsx; fecha del movimiento SAP.'},
 {id:'sap-4506693469',name:'Servicio de vigilancia del Sales Center Madrid',category:'Seguridad / cumplimiento',status:'Finalizado',date:'27/04/2025',provider:'MOVISTAR PROSEGUR ALARMAS S L',orderNumber:'4506693469',amount:'747,02 €',observations:'Importado de GASTOS 1316.xlsx; fecha del movimiento SAP.'},
 {id:'sap-4507026063',name:'Servicio de vigilancia del Sales Center Fuenlabrada',category:'Seguridad / cumplimiento',status:'Finalizado',date:'30/03/2026',provider:'MOVISTAR PROSEGUR ALARMAS S L',orderNumber:'4507026063',amount:'541,73 €',observations:'Importado de GASTOS 1316.xlsx; fecha del movimiento SAP.'},
 {id:'sap-4506719288',name:'Mantenimiento semestral de puerta basculante y medios de elevación',category:'Reparación / mantenimiento',status:'Finalizado',date:'14/05/2026',provider:'PROPERVALL SL',orderNumber:'4506719288',amount:'0,00 €',observations:'Importado de GASTOS 1316.xlsx; fecha del movimiento SAP.'},
 {id:'sap-4506628531',name:'Reparación de puerta basculante y alquiler de plataforma',category:'Reparación / mantenimiento',status:'Finalizado',date:'18/06/2025',provider:'PUERTAS AUTOMATICAS PORTIS SL',orderNumber:'4506628531',amount:'11,00 €',observations:'Importado de GASTOS 1316.xlsx; fecha del movimiento SAP.'},
 {id:'sap-4506639671',name:'Arrendamiento del Sales Center Madrid 2025 y 2026',category:'Otro',status:'Finalizado',date:'04/02/2025',provider:'SOCIEDAD ELECTRODOS Y FUNDENTES SLU',orderNumber:'4506639671',amount:'72011,25 €',observations:'Importado de GASTOS 1316.xlsx; fecha del movimiento SAP.'},
 {id:'sap-4900160502',name:'Fuente con garrafas de agua de 20 litros',category:'Suministro / consumible',status:'Finalizado',date:'17/07/2025',provider:'VIVA AQUA SERVICE SPAIN SA',orderNumber:'4900160502',amount:'12,64 €',observations:'Importado de GASTOS 1316.xlsx; fecha del movimiento SAP.'},
 {id:'sap-4400716211',name:'Guantes, gafas, calzado y chalecos de seguridad',category:'Seguridad / cumplimiento',status:'Finalizado',date:'07/02/2025',provider:'WATERFIRE SL',orderNumber:'4400716211',amount:'271,92 €',observations:'Importado de GASTOS 1316.xlsx; fecha del movimiento SAP.'}
];
...
function exportCurrentWorkbook(){
  ...
  const improvements=[['Concepto','Tipo','Fecha','Proveedor','N.º de solicitud','N.º de pedido','Importe sin IVA','Estado','Justificación','Actuación realizada','Resultado','Fecha de solicitud','Fecha de pedido','Fecha de finalización','Tipo de gasto','Actuación visible','Foto anterior','Foto posterior','Observaciones'],...improvementRows.map(r=>[r.name||'',r.category||'',r.date||'',r.provider||'',r.requestNumber||r.request||'',r.orderNumber||'',r.amount||'',r.status||'Necesidad detectada',r.justification||'',r.action||'',r.result||'',r.requestDate||'',r.orderDate||'',r.completionDate||'',r.expenseType||'',r.visible?'Sí':'No',r.beforeUrl||'',r.afterUrl||'',r.observations||''])];
...
function renderImprovements(){
 ...
 const rows=improvementRows.filter(row=>(filter==='all'||String(row.status||'Necesidad detectada')===filter)&&(year==='all'||String(row.date||'').endsWith(`/${year}`))&&[row.name,row.category,row.date,row.provider,row.requestNumber,row.request,row.orderNumber,row.amount,row.status,row.justification,row.action,row.result].join(' ').toLocaleLowerCase('es').includes(query));
...
async function saveImprovementEditor(){const name=sentenceCaseIfAllCaps($('improvement-edit-name').value.trim());if(!name){editorMessage($('improvement-editor'),'Escribe el concepto de la compra o mejora.');return}const changes={name,category:sentenceCaseIfAllCaps($('improvement-edit-category').value.trim()),status:$('improvement-edit-status').value,date:$('improvement-edit-date').value?dateFromEditor($('improvement-edit-date').value):'',detectionDate:$('improvement-edit-date').value?dateFromEditor($('improvement-edit-date').value):'',requestDate:$('improvement-edit-request-date').value?dateFromEditor($('improvement-edit-request-date').value):'',orderDate:$('improvement-edit-order-date').value?dateFromEditor($('improvement-edit-order-date').value):'',completionDate:$('improvement-edit-completion-date').value?dateFromEditor($('improvement-edit-completion-date').value):'',justification:$('improvement-edit-justification').value.trim(),action:$('improvement-edit-action').value.trim(),result:$('improvement-edit-result').value.trim(),provider:$('improvement-edit-provider').value.trim(),requestNumber:$('improvement-edit-request').value.trim(),orderNumber:$('improvement-edit-order').value.trim(),amount:$('improvement-edit-amount').value.trim(),expenseType:$('improvement-edit-expense').value,visible:$('improvement-edit-visible').checked,beforeUrl:$('improvement-edit-before').value.trim(),afterUrl:$('improvement-edit-after').value.trim(),observations:$('improvement-edit-observations').value.trim()};return commitEditor('improvements',improvementEditorIndex,changes,'improvement-editor',closeImprovementEditor)}
...
async function loadRemoteData(){
 ...
 dataLoadInFlight=(async()=>{try{
  ...
  const existingOrders=new Set(improvementRows.map(row=>String(row.orderNumber||'').trim()).filter(Boolean));
  const missingSapRows=SAP_IMPORT_ROWS.filter(row=>!existingOrders.has(row.orderNumber));
  if(missingSapRows.length){
   const payload=structuredClone(appDocument);payload.improvements=[...improvementRows,...missingSapRows];payload.updated=new Date().toISOString();payload.lastMutation=crypto.randomUUID();
