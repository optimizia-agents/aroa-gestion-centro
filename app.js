...
function exportCurrentWorkbook(){
  ...
  const improvements=[['Concepto','Tipo','Fecha','Proveedor','N.º de solicitud','N.º de pedido','Importe sin IVA','Estado','Justificación','Actuación realizada','Resultado','Fecha de detección','Fecha de finalización','Tipo de gasto','Foto anterior','Foto posterior','Observaciones'],...improvementRows.map(r=>[r.name||'',r.category||'',r.date||'',r.provider||'',r.requestNumber||r.request||'',r.orderNumber||'',r.amount||'',r.status||'Necesidad detectada',r.justification||'',r.action||'',r.result||'',r.detectionDate||'',r.completionDate||'',r.expenseType||'',r.beforeUrl||'',r.afterUrl||'',r.observations||''])];
...
async function saveImprovementEditor(){const name=sentenceCaseIfAllCaps($('improvement-edit-name').value.trim());if(!name){editorMessage($('improvement-editor'),'Escribe el concepto de la solicitud o pedido.');return}const changes={name,category:sentenceCaseIfAllCaps($('improvement-edit-category').value.trim()),status:$('improvement-edit-status').value,date:$('improvement-edit-date').value?dateFromEditor($('improvement-edit-date').value):'',detectionDate:$('improvement-edit-detection-date').value?dateFromEditor($('improvement-edit-detection-date').value):'',orderDate:$('improvement-edit-order-date').value?dateFromEditor($('improvement-edit-order-date').value):'',completionDate:$('improvement-edit-completion-date').value?dateFromEditor($('improvement-edit-completion-date').value):'',justification:$('improvement-edit-justification').value.trim(),action:$('improvement-edit-action').value.trim(),result:$('improvement-edit-result').value.trim(),provider:$('improvement-edit-provider').value.trim(),requestNumber:$('improvement-edit-request').value.trim(),orderNumber:$('improvement-edit-order').value.trim(),amount:$('improvement-edit-amount').value.trim(),expenseType:$('improvement-edit-expense').value,beforeUrl:$('improvement-edit-before').value.trim(),afterUrl:$('improvement-edit-after').value.trim(),observations:$('improvement-edit-observations').value.trim()};return commitEditor('improvements',improvementEditorIndex,changes,'improvement-editor',closeImprovementEditor)}
