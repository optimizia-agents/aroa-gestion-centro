...
async function loadRemoteData(){
 ...
 dataLoadInFlight=(async()=>{try{
  ...
  const normalized=moveEmbeddedPeopleToContacts(restored,providerContactsByName);
  if(JSON.stringify(normalized.center)!==JSON.stringify(centerRows)||JSON.stringify(normalized.contacts)!==JSON.stringify(providerContactsByName)){const payload=structuredClone(appDocument);payload.center=normalized.center;payload.providerContacts=normalized.contacts;payload.updated=new Date().toISOString();payload.lastMutation=crypto.randomUUID();try{const confirmed=await writeDocument('appState','main',payload,appRevision);applyConfirmedDocument(confirmed)}catch(error){console.warn('No se pudieron normalizar los contactos internos',error)}}
  return true;
