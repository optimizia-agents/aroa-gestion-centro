...
const previousClientAwareLoad=loadRemoteData;loadRemoteData=async function(){await previousClientAwareLoad();try{const d=await kurroRequest({api:'data'});loadRemoteClients(d.clients);const c=await kurroRequest({api:'config'});saveRemoteKurroLists(c.values);refreshKurroPeopleOptions();refreshClientOptions();renderDashboard()}catch(e){}};
