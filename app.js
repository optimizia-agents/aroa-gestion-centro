...
function renderImprovements(){
 ...
 body.innerHTML=rows.length?rows.map(row=>{const index=improvementRows.indexOf(row),photos=[row.beforeUrl&&`<a class="evidence-link" href="${htmlEscape(row.beforeUrl)}" target="_blank" rel="noopener">Antes</a>`,row.afterUrl&&`<a class="evidence-link" href="${htmlEscape(row.afterUrl)}" target="_blank" rel="noopener">Después</a>`].filter(Boolean).join(' ');return `<tr><td>${htmlEscape(row.name||'Sin indicar')}</td><td>${htmlEscape(row.category||'Sin indicar')}</td><td class="date">${htmlEscape(row.orderDate||row.date||'Sin fecha')}</td><td>${htmlEscape(row.provider||'Sin indicar')}</td><td class="copyable">${htmlEscape(row.requestNumber||row.request||'')}</td><td class="copyable">${htmlEscape(row.orderNumber||'')}</td><td class="copyable">${htmlEscape(row.supplierNumber||'')}</td><td>${htmlEscape(improvementAmount(row.amount))}</td><td><span class="status ${improvementStatusClass(row.status)}">${htmlEscape(row.status||'Necesidad detectada')}</span></td><td><button class="edit-btn" type="button" onclick="openImprovementEditor(${index})">Editar</button>${photos?`<div class="improvement-photo-links">${photos}</div>`:''}</td></tr>`}).join(''):'<tr><td colspan="10" class="empty">No hay mejoras que coincidan con los filtros.</td></tr>';
