...
function kurroOptions(kind){const lists=mergedKurroLists(),source=kind==='category'?centerRows.map(r=>r.category):kind==='owner'?centerRows.map(r=>r.owner):kind==='provider'?centerRows.map(r=>r.provider):pendingRows.map(r=>r.person);return [...new Set([...(lists[kind]||[]),...source].map(v=>String(v||'').trim()).filter(Boolean))].sort((a,b)=>a.localeCompare(b,'es'))}
function fillKurroSelect(id,values,current=''){const select=$(id);if(!select)return;const value=current||select.value;const prefix=id==='edit-category'?'<option value="">Sin categoría</option>':id==='edit-owner'?'<option value="">Sin asignar</option>':id==='edit-provider'?'<option value="Ninguno">Ninguno</option>':'';select.innerHTML=prefix+values.filter(v=>v!=='Ninguno').map(v=>`<option value="${String(v).replaceAll('&','&amp;').replaceAll('"','&quot;')}">${String(v).replaceAll('&','&amp;').replaceAll('<','&lt;').replaceAll('>','&gt;')}</option>`).join('');if(value&&values.includes(value))select.value=value;else if(id==='edit-provider'&&!value)select.value='Ninguno'}
