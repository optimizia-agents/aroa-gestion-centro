...
function toggleProviderContactForm(show){const form=$('provider-contact-new');if(form)form.hidden=!show;if(show)$('provider-contact-name')?.focus()}
...
async function saveProviderContact(){const provider=canonicalProvider($('edit-provider')?.value),name=$('provider-contact-name')?.value.trim(),phone=$('provider-contact-phone')?.value.trim()||'',email=$('provider-contact-email')?.value.trim()||'';if(!provider||provider==='Ninguno'){editorMessage($('center-editor'),'Elige primero un proveedor externo.');return}if(!name){editorMessage($('center-editor'),'Escribe el nombre del contacto.');return}if(!firebaseUser||!appRevision){editorMessage($('center-editor'),'Actualiza los datos de Firebase antes de guardar.');return}const contacts=selectedProviderContacts(),key=`${name.toLocaleLowerCase('es')}|${phone.toLocaleLowerCase('es')}|${email.toLocaleLowerCase('es')}`,existing=contacts.find(c=>`${c.name.toLocaleLowerCase('es')}|${(c.phone||'').toLocaleLowerCase('es')}|${(c.email||'').toLocaleLowerCase('es')}`===key);if(existing){refreshProviderContacts(existing.id);toggleProviderContactForm(false);editorMessage($('center-editor'),'Ese contacto ya está en la lista de este proveedor.');return}const contact={id:crypto.randomUUID(),name,phone,email};const payload=structuredClone(appDocument);payload.providerContacts=structuredClone(providerContactsByName);payload.providerContacts[provider]=[...contacts,contact];payload.updated=new Date().toISOString();payload.lastMutation=crypto.randomUUID();try{const confirmed=await writeDocument('appState','main',payload,appRevision);applyConfirmedDocument(confirmed);refreshProviderContacts(contact.id);toggleProviderContactForm(false);$('provider-contact-name').value='';$('provider-contact-phone').value='';$('provider-contact-email').value='';$('center-editor').dataset.dirty='true';editorMessage($('center-editor'),'Contacto guardado para '+provider+'. Guarda la actividad para asociarlo.')}catch(error){editorMessage($('center-editor'),error.message==='CONFLICT'?'Los datos han cambiado. Pulsa Actualizar y vuelve a intentarlo.':'No se ha podido guardar el contacto. Comprueba la conexión y vuelve a intentarlo.')}}
...
function exportCurrentWorkbook(){
  ...
  const center=[['Actividad','Categoría','Periodicidad','Última revisión','Próxima revisión','Responsable','Estado','Comentarios'],...centerRows.map(r=>[r.activity||'',r.category||'',canonicalFrequency(r.frequency),r.last||'',r.next||'',r.owner||'',statusLabel(effectiveStatus(r)),r.action||''])];
