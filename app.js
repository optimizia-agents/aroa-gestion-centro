...
function renderImprovements(){
 ...
 const rows=improvementRows.filter(row=>(filter==='all'||String(row.status||'Necesidad detectada')===filter)&&(year==='all'||String(row.orderDate||row.date||'').endsWith(`/${year}`))&&[row.name,row.category,row.orderDate,row.date,row.provider,row.supplierNumber,row.requestNumber,row.request,row.orderNumber,row.amount,row.status,row.justification,row.action,row.result].join(' ').toLocaleLowerCase('es').includes(query));
