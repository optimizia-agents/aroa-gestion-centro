...
function purchaseProviderContactGroup(provider){const raw=canonicalProvider(provider);if(!raw)return'';const key=Object.keys(providerContactsByName||{}).find(name=>String(name).toLocaleLowerCase('es')===raw.toLocaleLowerCase('es'));return key||raw}
...
function purchaseProviderOptions(){const values=[...(remoteKurroLists.provider||[]),...centerRows.map(r=>r.provider),...improvementRows.map(r=>r.provider)].map(v=>String(v||'').trim()).filter(Boolean);return [...new Map(values.map(v=>[v.toLocaleLowerCase('es'),v])).values()].sort((a,b)=>a.localeCompare(b,'es'))}
