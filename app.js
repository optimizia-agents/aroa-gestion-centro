...
function remotePendingRow(row,index,person,fileId){return{person,status:String(row[0]||'PENDIENTE').toUpperCase(),comments:row[1]||'',text:row[2]||'',priority:String(row[3]||'NORMAL').toUpperCase(),date:row[4]||'',updated:row[5]||'',closed:row[6]||'',serverRow:index+2,fileId}}
...
function setSyncState(state){const label=$('sync-label'),dot=document.querySelector('.sync-dot');if(!label)return;label.classList.toggle('sync-error',state==='error');label.classList.toggle('sync-loading',state==='loading');label.textContent=state==='loading'?'Actualizando…':state==='error'?'Sin conexión':'Sincronizado';if(dot)dot.style.background=state==='error'?'#d66045':state==='loading'?'#e3b34d':'#65c99a'}
...
const CLIENTS_FILE_ID='1hSQQSyeWiJ03Hf76s0X4pbYANMDAUwygN6pOUbQNNtY';
