...
async function refreshKURROFromSheets(silent=false){if(kurroRefreshInFlight||kurroPendingWrites||isEditingKURRO())return false;kurroRefreshInFlight=true;try{const ok=await loadRemoteData();if(!silent&&ok)showSyncToast(firebaseUser?'Datos actualizados desde Firebase':'Datos actualizados');return ok}finally{kurroRefreshInFlight=false}}
...
localStorage.removeItem('aroa_firebase_id_token');localStorage.removeItem('aroa_firebase_refresh_token');
...
async function restRead(){const r=await restFetch('https://firestore.googleapis.com/v1/projects/'+FIREBASE_CONFIG.projectId+'/databases/(default)/documents/appState/main');if(r.status===404)return null;if(!r.ok)throw new Error('FIRESTORE_GET_'+r.status);const doc=await r.json();return fsDecode({mapValue:{fields:doc.fields||{}}})}
...
async function submitFirebaseRest(mode){const password=$('firebase-password')?.value||'';if(password.length<6){firebaseAuthMessage('La contraseña debe tener al menos 6 caracteres.');return}try{firebaseAuthMessage(mode==='create'?'Creando tu acceso…':'Entrando…');await restAuth(mode,password);closeFirebaseAuth(true);await finishFirebaseRest()}catch(error){const code=String(error.code||'');firebaseAuthMessage(code.includes('EMAIL_EXISTS')?'Ese acceso ya existe. Pulsa Entrar.':code.includes('INVALID_PASSWORD')||code.includes('EMAIL_NOT_FOUND')||code.includes('INVALID_LOGIN_CREDENTIALS')?'La contraseña no es correcta o el acceso aún no existe.':code.includes('INVALID_API_KEY')?'La clave de Firebase no está autorizada para esta web.':code.includes('OPERATION_NOT_ALLOWED')?'El acceso por correo y contraseña no está habilitado.':'No se ha podido completar el acceso: '+(code||'error de conexión'))}}
function addFirebaseRestUI(){if($('firebase-auth'))return;document.body.insertAdjacentHTML('beforeend',`<div id="firebase-auth" class="modal" aria-hidden="true"><div class="modal-card pending-editor-card" role="dialog" aria-modal="true"><div class="modal-heading"><div><p class="eyebrow">ACCESO PRIVADO</p><h2>Entrar en Aroa Gestión</h2></div><button class="modal-close" type="button" aria-label="Cerrar" id="firebase-auth-close">×</button></div><p class="pending-editor-note">Usa el acceso de Optimizia para abrir y guardar tus datos en Firebase.</p><label class="editor-grid" style="display:grid;gap:6px;color:#557078;font-size:13px;font-weight:700">Correo electrónico<input value="${FIREBASE_ALLOWED_EMAIL}" disabled></label><label style="display:grid;gap:6px;margin-top:14px;color:#557078;font-size:13px;font-weight:700">Contraseña<input id="firebase-password" type="password" autocomplete="current-password" placeholder="Mínimo 6 caracteres"></label><p id="firebase-auth-message" class="pending-editor-note" aria-live="polite"></p><div class="modal-actions"><button class="secondary" type="button" id="firebase-auth-cancel">Cancelar</button><button class="secondary" type="button" id="firebase-auth-create">Crear mi acceso</button><button class="primary" type="button" id="firebase-auth-login">Entrar</button></div></div></div>`);$('firebase-auth-close').addEventListener('click',closeFirebaseAuth);$('firebase-auth-cancel').addEventListener('click',closeFirebaseAuth);$('firebase-auth-login').addEventListener('click',()=>submitFirebaseRest('login'));$('firebase-auth-create').addEventListener('click',()=>submitFirebaseRest('create'));const top=document.querySelector('.top-actions');if(top&&!$('firebase-auth-open')){const button=document.createElement('button');button.id='firebase-auth-open';button.className='refresh-button';button.textContent='Acceso privado';button.addEventListener('click',openFirebaseAuth);top.insertBefore(button,top.firstChild)}}
async function finishFirebaseRest(){firebaseUser={email:FIREBASE_ALLOWED_EMAIL};try{const existing=await restRead();if(existing){centerRows.splice(0,centerRows.length,...(existing.center||[]));pendingRows.splice(0,pendingRows.length,...(existing.pending||[]));clientRows.splice(0,clientRows.length,...(existing.clients||[]));if(existing.lists)remoteKurroLists=existing.lists;saveData();saveClientData()}else{await legacyLoadRemoteData();await restWrite(firebaseSnapshot())}try{if(await restoreOwnersFromLegacySource())await firebasePersistSnapshot()}catch(error){console.warn('No se pudo reconstruir Responsable',error)}await loadDirectoryFromFirebase();firebaseSnapshot;refreshKURROMetrics();renderCenter();renderPending();renderClients();if($('sync-label'))$('sync-label').textContent='Firebase · sincronizado';setDataAlert('');unlockPrivateApp()}catch(error){firebaseUser=null;if($('sync-label'))$('sync-label').textContent='Acceso privado';setDataAlert('Firebase no ha podido sincronizarse. Vuelve a intentarlo; los datos locales no se han borrado.');openFirebaseAuth()}}
...
async function firebaseDataForCurrentSession(){if(firebaseDb){const result=await firebaseData();await loadDirectoryFromFirebase();return result}const existing=await restRead();if(existing){centerRows.splice(0,centerRows.length,...(existing.center||[]));pendingRows.splice(0,pendingRows.length,...(existing.pending||[]));clientRows.splice(0,clientRows.length,...(existing.clients||[]));if(existing.lists)remoteKurroLists=existing.lists;saveData();saveClientData()}await loadDirectoryFromFirebase();return firebaseDataFromRows()}
loadGvizData=async function(){if(firebaseUser)return firebaseDataForCurrentSession();return legacyLoadGvizData()};
kurroRequest=async function(params){if(firebaseUser){if(params.api==='data')return firebaseDataForCurrentSession();await firebasePersistSnapshot();return{ok:true,result:{firebase:true}}}return legacyKurroRequest(params)};
async function startFirebaseRest(){addFirebaseRestUI();openFirebaseAuth()}
startFirebaseRest();
