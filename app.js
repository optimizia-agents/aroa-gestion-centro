...
function renderProviders(){const body=$('provider-table');if(!body)return;const q=String($('provider-search')?.value||'').trim().toLocaleLowerCase('es');const rows=providerDirectoryRows().filter(row=>[row.name,row.supplierNumber,row.description].join(' ').toLocaleLowerCase('es').includes(q));body.innerHTML=rows.length?rows.map(row=>{const contacts=(providerContactsByName[purchaseProviderContactGroup(row.name)]||[]);return `<tr><td><span class="provider-directory-name">${htmlEscape(row.name)}</span></td><td>${row.supplierNumber?`<span class="copyable copyable-id" role="button" tabindex="0" data-copy-value="${htmlEscape(row.supplierNumber)}" title="Copiar número de proveedor">${htmlEscape(row.supplierNumber)}</span>`:'Sin indicar'}</td><td class="provider-directory-contacts">${contacts.length?`${contacts.length} contacto${contacts.length===1?'':'s'}`:'Sin contactos'}</td><td>${htmlEscape(row.description||PROVIDER_DESCRIPTIONS[canonicalProvider(row.name)]||'Pendiente de clasificar')}</td><td><button type="button" class="edit-btn" onclick="openProviderEditor('${htmlEscape(providerDirectoryKey(row.name))}')">Editar</button></td></tr>`}).join(''):'<tr><td colspan="5" class="empty">No hay proveedores que coincidan con la búsqueda.</td></tr>';$('provider-count').textContent=`${rows.length} proveedores`}
