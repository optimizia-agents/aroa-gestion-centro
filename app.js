...
function renderImprovements(){
 const query=String($('improvement-search')?.value||'').trim().toLocaleLowerCase('es'),category=$('improvement-category-filter')?.value||'all',filter=$('improvement-status-filter')?.value||'all',year=$('improvement-year-filter')?.value||'all';
 ...
 const rows=improvementRows.filter(row=>(category==='all'||String(row.category||'').trim()===category)&&(filter==='all'||String(row.status||'Necesidad detectada')===filter)&&(year==='all'||String(row.orderDate||row.date||'').endsWith(`/${year}`))&&[row.name,row.category,row.orderDate,row.date,row.provider,row.supplierNumber,row.requestNumber,row.request,row.orderNumber,row.amount,row.status,row.justification,row.action,row.result].join(' ').toLocaleLowerCase('es').includes(query)).sort((a,b)=>{const ao=String(a.orderNumber||'').trim(),bo=String(b.orderNumber||'').trim();if(!ao&&!bo)return 0;if(!ao)return 1;if(!bo)return -1;const an=Number(ao),bn=Number(bo);return Number.isFinite(an)&&Number.isFinite(bn)?an-bn:ao.localeCompare(bo,'es',{numeric:true,sensitivity:'base'})});
...
function renderEconomicAnalysis(){
 ...
 const periodTitle=$('analysis-year-title');
 if(selectedAnalysisYear!=='all'){
  ...
  if(periodTitle)periodTitle.textContent=`Evolución mensual de ${selectedAnalysisYear}`;
 }else if(periodTitle)periodTitle.textContent='Evolución por año';
