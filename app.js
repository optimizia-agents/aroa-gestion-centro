...
const renderPendingWithPersonSelect=renderPending;renderPending=function(){refreshPendingPersonSelect();renderPendingWithPersonSelect()};document.querySelector('#pending-person')?.addEventListener('change',event=>{window.person=event.target.value;renderPending()});function resetPendingFilters(){if($('pending-search'))$('pending-search').value='';if($('pending-person'))$('pending-person').value='all';if($('pending-priority'))$('pending-priority').value='all';if($('pending-status'))$('pending-status').value='PENDIENTE';window.person='all';renderPending();if(typeof saveViewFilters==='function')saveViewFilters('pending')};refreshPendingPersonSelect();renderPending();
