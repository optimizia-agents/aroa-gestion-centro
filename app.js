...
function addKurroOption(kind,selectId,label){const name=prompt(label);if(!name)return;const clean=kind==='provider'?canonicalProvider(name):name.trim();if(!clean)return;const lists=savedKurroLists();lists[kind]=[...(lists[kind]||[]),clean];saveKurroLists(lists);refreshKurroPeopleOptions();$(selectId).value=clean;showSyncToast(`${clean} añadido a la lista`)}
...
function selectedProviderContacts(){const provider=canonicalProvider($('edit-provider')?.value);return provider?providerContactsByName[provider]||[]:[]}
...
async function saveProviderContact(){const provider=canonicalProvider($('edit-provider')?.value),name=$('provider-contact-name')?.value.trim(),phone=$('provider-contact-phone')?.value.trim()||'',email=$('provider-contact-email')?.value.trim()||'',editingId=providerContactEditingId;if(!provider||provider==='Ninguno'){editorMessage($('center-editor'),'Elige primero un proveedor externo.');return}if(!name){editorMessage($('center-editor'),'Escribe el nombre del contacto.');return}if(!firebaseUser||!appRevision){editorMessage($('center-editor'),'Actualiza los datos de Firebase antes de guardar.');return}const contacts=selectedProviderContacts(),key=`${name.toLocaleLowerCase('es')}|${phone.toLocaleLowerCase('es')}|${email.toLocaleLowerCase('es')}`,existing=contacts.find(c=>c.id!==editingId&&`${c.name.toLocaleLowerCase('es')}|${(c.phone||'').toLocaleLowerCase('es')}|${(c.email||'').toLocaleLowerCase('es')}`===key);if(existing){refreshProviderContacts(existing.id);toggleProviderContactForm(false);editorMessage($('center-editor'),'Ese contacto ya está en la lista de este proveedor.');return}const original=contacts.find(c=>c.id===editingId),contact=editingId&&original?{...original,name,phone,email}:{id:crypto.randomUUID(),name,phone,email};const payload=structuredClone(appDocument);payload.providerContacts=structuredClone(providerContactsByName);payload.providerContacts[provider]=editingId?contacts.map(c=>c.id===editingId?contact:c):[...contacts,contact];payload.updated=new Date().toISOString();payload.lastMutation=crypto.randomUUID();try{const confirmed=await writeDocument('appState','main',payload,appRevision);applyConfirmedDocument(confirmed);providerContactEditingId='';refreshProviderContacts(contact.id);toggleProviderContactForm(false);$('center-editor').dataset.dirty='true';editorMessage($('center-editor'),(editingId?'Contacto actualizado':'Contacto guardado')+' para '+provider+'. Guarda la actividad para asociarlo.')}catch(error){editorMessage($('center-editor'),error.message==='CONFLICT'?'Los datos han cambiado. Pulsa Actualizar y vuelve a intentarlo.':'No se ha podido guardar el contacto. Comprueba la conexión y vuelve a intentarlo.')}}
...
function syncProviderField(){const field=$('edit-provider-field'),contactField=$('provider-contact-field'),type=$('edit-type'),input=$('edit-provider');if(!field||!type)return;const external=type.value==='EXTERNO';field.hidden=!external;if(input){input.disabled=!external;if(!external)input.value='Ninguno'}if(contactField)contactField.hidden=!external||!input?.value||input.value==='Ninguno';if(!external)toggleProviderContactForm(false);refreshProviderContacts(external?$('edit-provider-contact')?.value:'')}
...
openCenterEditor=function(index){_openCenterEditor(index);setCenterEvidenceDraft(centerRows[index])};
...
function renderClients(){
  ...
  table.innerHTML=rows.length?rows.map(row=>{
    ...
    const statusHtml=row.status==='REALIZADO'?'<span class="status done">REALIZADO</span>':row.status==='EN PROCESO'?'<span class="status process">EN PROCESO</span>':'<span class="status open">PENDIENTE</span>';
...
function renderPending(){
  ...
  table.innerHTML=rows.length?rows.map(r=>{
    ...
    const status=r.status==='REALIZADO'?'<span class="status done">REALIZADO</span>':r.status==='EN PROCESO'?'<span class="status process">EN PROCESO</span>':'<span class="status open">PENDIENTE</span>';
...
async function saveCenterEditorFlexible(){
 ...
 const providerValue=$('edit-provider')?.value.trim()||'';const changes={activity:$('edit-activity').value.trim(),category:$('edit-category').value.trim()||'Otros',frequency,last:$('edit-last').value?dateFromEditor($('edit-last').value):'',next:$('edit-next').value?dateFromEditor($('edit-next').value):'',owner:$('edit-owner').value.trim(),type:$('edit-type')?.value||'INTERNO',provider:providerValue==='Ninguno'?'':canonicalProvider(providerValue),providerContactId:$('edit-type')?.value==='EXTERNO'?$('edit-provider-contact')?.value||'':'',action:$('edit-action').value,status:$('edit-status').value};
