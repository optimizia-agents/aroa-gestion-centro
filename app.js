...
async function saveCenterEditorFlexible(){
 ...
 const changes={activity:$('edit-activity').value.trim(),category:$('edit-category').value.trim()||'Otros',frequency,last:$('edit-last').value?dateFromEditor($('edit-last').value):'',next:$('edit-next').value?dateFromEditor($('edit-next').value):'',owner:$('edit-owner').value.trim(),type:$('edit-type')?.value||'INTERNO',provider:$('edit-provider')?.value.trim()||'',action:$('edit-action').value,status:$('edit-status').value};
