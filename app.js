...
function parseMoneyValue(value){
 ...
 let normalized=raw;
 if(lastComma>=0&&lastDot>=0){const decimal=lastComma>lastDot?',':'.';const thousands=decimal===','?'.':',';normalized=raw.split(thousands).join('').replace(decimal,'.');}
 else if(lastComma>=0||lastDot>=0){const separator=lastComma>=0?',':'.';const parts=raw.split(separator);const decimals=parts[parts.length-1];normalized=parts.length===2&&decimals.length===3?parts.join(''):raw.replace(separator,'.');}
 const amount=Number(normalized);return Number.isFinite(amount)?amount:null;
}
function formatMoneyValue(value){const amount=parseMoneyValue(value);return amount===null?'':new Intl.NumberFormat('es-ES',{minimumFractionDigits:2,maximumFractionDigits:2}).format(amount)+' €'}
const SAP_IMPORT_ROWS = [];
const SAP_DOCUMENT_DATES = {"4507105681":"16/09/2026","4507105680":"16/09/2026","4507007736":"11/03/2026","4506639671":"01/02/2025","4507026063":"01/01/2026","4506645062":"15/02/2025","4900160502":"30/06/2025","4400799587":"21/07/2026","4507123046":"01/07/2026","4507120107":"01/07/2026","4507119313":"01/07/2026","4507116677":"01/07/2026","4507112970":"01/07/2026","4507085738":"26/06/2026","4506670015":"30/04/2025","4506719288":"04/05/2026","4507030154":"29/04/2026","4506941948":"29/04/2026","4507036077":"20/04/2026","4507018162":"01/04/2026","4506872953":"02/02/2026","4506873736":"02/02/2026","4506943957":"02/02/2026","4506974317":"31/01/2026","4506816352":"12/02/2025","4506664771":"17/03/2025","4506638772":"19/02/2025","4506693469":"18/03/2025","4506918610":"26/11/2025","4506861372":"16/10/2025","4506887082":"27/10/2025","4506794695":"25/08/2025","4506797354":"19/08/2025","4506699527":"16/07/2025","4506794692":"16/07/2025","4506783314":"30/06/2025","4506783431":"19/06/2025","4506688747":"13/06/2025","4506579316":"20/05/2025","4506747572":"10/05/2025","4506803505":"08/05/2025","4506724123":"08/05/2025","4506700813":"10/04/2025","4506582992":"10/03/2025","4506656885":"28/02/2025","4506657049":"28/02/2025","4506660933":"28/02/2025","4506660901":"28/02/2025","4506647112":"27/02/2025","4506643372":"24/02/2025","4506628531":"07/02/2025"};
...
let providerModalContactEditingId="";
...
function canonicalProvider(value){const raw=String(value||'').trim().replace(/\s+/g,' ');if(!raw)return'';const key=raw.toLocaleLowerCase('es');const known={'propervall':'PROPERVALL SL','propervall sl':'PROPERVALL SL','anticymex':'ANTICIMEX 3D SANIDAD AMBIENTAL SA','anticimex 3d sanidad ambiental sa':'ANTICIMEX 3D SANIDAD AMBIENTAL SA','apave':'APAVE INSPECTION SPAIN SA (EUROCONTROL)','apave inspection spain sa':'APAVE INSPECTION SPAIN SA (EUROCONTROL)','apave inspection spain sa / eurocontrol':'APAVE INSPECTION SPAIN SA (EUROCONTROL)','eurocontrol sa':'APAVE INSPECTION SPAIN SA (EUROCONTROL)','eurocontrol sa, grupo apave':'APAVE INSPECTION SPAIN SA (EUROCONTROL)','cyrasa':'CYRASA SEGURIDAD SL','cyrasa seguridad sl':'CYRASA SEGURIDAD SL','iss facility':'ISS FACILITY SERVICES SA','iss facility services sa':'ISS FACILITY SERVICES SA','linde':'LINDE MATERIAL HANDLING IBERICA, SA','linde material handling iberica, sa':'LINDE MATERIAL HANDLING IBERICA, SA','linde material handling ibérica, sa':'LINDE MATERIAL HANDLING IBERICA, SA','movistar prosegur alarmas s l':'MOVISTAR PROSEGUR ALARMAS SL','movistar prosegur alarmas sl':'MOVISTAR PROSEGUR ALARMAS SL','riscat':'RISC i SEGURETAT RISCAT, S.L.','risc i seguretat riscat, s.l.':'RISC i SEGURETAT RISCAT, S.L.','vitaly':'VITALIS VIDA EN EL TRABAJO, S.L.','vitalis vida en el trabajo, s.l.':'VITALIS VIDA EN EL TRABAJO, S.L.','ayto. de fuenlabrada':'AYUNTAMIENTO DE FUENLABRADA','ayto de fuenlabrada':'AYUNTAMIENTO DE FUENLABRADA'};return (known[key]||raw).toLocaleUpperCase('es').replace(/\bS\s+L\s+U\b/g,'SLU').replace(/\bS\s+L\b/g,'SL').replace(/\bS\.L\.U?\b/g,'SLU').replace(/\bS\.L\.?\b/g,'SL').replace(/\b(SL|SA|SLU)\.(?=\s|$)/g,'$1')}
const PROVIDER_DESCRIPTIONS={'ANTICIMEX 3D SANIDAD AMBIENTAL SA':'Control de plagas y tratamientos de sanidad ambiental.','APAVE INSPECTION SPAIN SA (EUROCONTROL)':'Inspecciones reglamentarias y seguridad industrial: portón, APQ y equipos.','AYUNTAMIENTO DE FUENLABRADA':'Licencias, trámites y tasas municipales.','BECHTLE S L U':'Equipamiento y material informático.','BRICOLAJES MARQUINA S L':'Suministros y reparaciones de mantenimiento.','CVA':'Formación y coordinación de operaciones CVA.','CYRASA SEGURIDAD SL':'Revisiones y mantenimiento de PCI, extintores, BIE y sistemas contra incendios.','DIAGONAL INTERNATIONAL MOVING, S.L MUDANZAS DIAGONAL':'Mudanzas y traslados.','DOMENECH PAPER SL':'Suministro de papel y consumibles.','E COORDINA GESTION DOCUMENTAL S L':'Coordinación de actividades empresariales (CAE).','ESPAIS DE SALUT ROVIRA S L':'Servicios de salud laboral y prevención.','ESTRUCTURAS METALICAS SERRANO E HIJOS SL':'Fabricación e instalación de estructuras, plataformas y protecciones metálicas.','EUROCONTROL SA':'Inspecciones reglamentarias y seguridad industrial.','FIC SUMINISTROS, ALQUILERES Y SERVI TÉCNICOS PARA LA INDUSTRIA, S.L.':'Suministros, alquileres y servicio técnico industrial.','GRUPO SOLUCION 10 MADRID SL':'Servicio puntual del centro; pendiente de concretar.','IPAT IBERICA S L':'Servicio técnico y actuaciones de seguridad; pendiente de concretar.','ISS FACILITY SERVICES SA':'Limpieza de oficinas, baños, nave y cristales.','KALAMAZOO PRODUCTOS DE OFICINA SL':'Material de oficina y consumibles.','LINDE MATERIAL HANDLING IBERICA, SA':'Mantenimiento y asistencia de la carretilla elevadora LINDE E25.','MANTENIMIENTOS IND. DE LA SAGRA SLL':'Mantenimiento industrial y reparaciones.','MOVISTAR PROSEGUR ALARMAS SL':'Vigilancia, alarmas y seguridad del centro.','NOVODINAMICA SLU':'Equipamiento y material del centro.','ORION PREVENCION SL':'Formación y servicios de prevención.','PROPERVALL SL':'Mantenimiento de instalaciones, portones y medios de elevación.','PUERTAS AUTOMATICAS PORTIS SL':'Reparación y mantenimiento de puertas automáticas.','RISC i SEGURETAT RISCAT, S.L.':'MAUs, ATEX y documentación de seguridad.','SOCIEDAD ELECTRODOS Y FUNDENTES SLU':'Arrendamiento del Sales Center.','VITALIS VIDA EN EL TRABAJO, S.L.':'Vigilancia de la salud, formación e informes preventivos.','VIVA AQUA SERVICE SPAIN SA':'Suministro de agua.','WATERFIRE SL':'Equipos de protección individual (EPI).'};
function providerDescription(name){const key=canonicalProvider(name);return PROVIDER_DESCRIPTIONS[key]||Object.entries(PROVIDER_DESCRIPTIONS).find(([label])=>canonicalProvider(label)===key)?.[1]||''}

// El catálogo de Proveedores es la referencia común. Al cargar datos se agrupan
// las variantes históricas y se conservan los contactos asociados a cada una.
function normalizeProviderData(){
 const normalizeRows=rows=>rows.map(row=>{const provider=canonicalProvider(row.provider);return provider&&provider!==row.provider?{...row,provider}:row});
 const nextCenter=normalizeRows(centerRows),nextImprovements=normalizeRows(improvementRows);
 const nextLists=structuredClone(remoteKurroLists||{});
 const names=[...(nextLists.provider||[]),...nextCenter.map(row=>row.provider),...nextImprovements.map(row=>row.provider),...(providerDirectory||[]).map(row=>row.name)].map(canonicalProvider).filter(Boolean);
 nextLists.provider=[...new Map(names.map(name=>[name.toLocaleLowerCase('es'),name])).values()].sort((a,b)=>a.localeCompare(b,'es'));
 const nextDirectory=[];const directoryMap=new Map();
 const addDirectory=(row={})=>{const name=canonicalProvider(row.name||row.provider);if(!name)return;const key=name.toLocaleLowerCase('es');const current=directoryMap.get(key)||{name};const merged={...current,...row,name};if(!merged.supplierNumber){const source=nextImprovements.find(item=>canonicalProvider(item.provider)===name&&item.supplierNumber);if(source)merged.supplierNumber=String(source.supplierNumber).trim()}if(!merged.description)merged.description=providerDescription(name);directoryMap.set(key,merged)};
 nextLists.provider.forEach(name=>addDirectory({name}));(providerDirectory||[]).forEach(addDirectory);nextDirectory.push(...directoryMap.values());
 const nextContacts={};const contactSeen={};Object.entries(providerContactsByName||{}).forEach(([raw,contacts])=>{const group=raw===INTERNAL_CONTACTS_KEY?INTERNAL_CONTACTS_KEY:canonicalProvider(raw);if(!group)return;nextContacts[group] ||= [];contactSeen[group] ||= new Set();(Array.isArray(contacts)?contacts:[]).forEach(contact=>{const key=[contact.id,contact.name,contact.phone,contact.email].map(value=>String(value||'').toLocaleLowerCase('es')).join('|');if(!contactSeen[group].has(key)){nextContacts[group].push(contact);contactSeen[group].add(key)}})});
 const changed=JSON.stringify(nextCenter)!==JSON.stringify(centerRows)||JSON.stringify(nextImprovements)!==JSON.stringify(improvementRows)||JSON.stringify(nextLists)!==JSON.stringify(remoteKurroLists)||JSON.stringify(nextDirectory)!==JSON.stringify(providerDirectory)||JSON.stringify(nextContacts)!==JSON.stringify(providerContactsByName);
 return {changed,center:nextCenter,improvements:nextImprovements,lists:nextLists,directory:nextDirectory,contacts:nextContacts};
}

function kurroOptions(kind){const lists=mergedKurroLists(),source=kind==='category'?centerRows.map(r=>r.category):kind==='owner'?centerRows.map(r=>r.owner):kind==='provider'?[...providerDirectoryRows().map(row=>row.name),...centerRows.map(r=>r.provider),...improvementRows.map(r=>r.provider)]:pendingRows.map(r=>r.person);const values=[...(lists[kind]||[]),...source].map(v=>kind==='provider'?canonicalProvider(v):kind==='owner'?canonicalOwner(v):kind==='category'?canonicalCategory(v):String(v||'').trim()).filter(v=>kind!=='category'||!retiredCenterCategories.includes(v)).filter(Boolean);return [...new Map(values.map(v=>[kind==='provider'||kind==='owner'?v.toLocaleLowerCase('es'):v,v])).values()].sort((a,b)=>a.localeCompare(b,'es'))}
...
function selectedProviderContacts(){const group=selectedContactGroup();if(!group)return[];return group===INTERNAL_CONTACTS_KEY?(providerContactsByName[group]||[]):purchaseProviderContacts(group)}
...
function openPendingEditor(index=-1){$('delete-pending-editor').hidden=index<0;refreshKurroPeopleOptions();editorMessage($('pending-editor'),'');pendingEditorIndex=index;$('pending-edit-text').value=index<0?'':pendingRows[index].text||'';$('pending-edit-person').value=index<0?(window.person!=='all'?window.person:kurroOptions('person')[0]||''):pendingRows[index].person;$('pending-edit-priority').value=index<0?'NORMAL':pendingRows[index].priority||'NORMAL';$('pending-edit-status').value=index<0?'PENDIENTE':pendingRows[index].status||'PENDIENTE';$('pending-edit-date').value=index<0?'':pendingDateForEditor(pendingRows[index].date);$('pending-edit-updated').value=index<0?pendingDateForEditor(formatDate(new Date())):pendingDateForEditor(pendingRows[index].updated);$('pending-edit-comments').value=index<0?'':pendingRows[index].comments||'';$('pending-editor-title').textContent=index<0?'Nuevo seguimiento':'Editar seguimiento';$('pending-editor').classList.add('open');$('pending-editor').setAttribute('aria-hidden','false')}
...
async function copyImprovementValue(value){try{if(navigator.clipboard?.writeText)await navigator.clipboard.writeText(value);else{const area=document.createElement('textarea');area.value=value;area.style.position='fixed';area.style.opacity='0';document.body.appendChild(area);area.select();document.execCommand('copy');area.remove()}showSyncToast('Copiado al portapapeles')}catch(error){showSyncToast('No se ha podido copiar')}}
...
document.querySelectorAll('.nav-item').forEach(button=>button.addEventListener('click',()=>{
 const titles={center:'Control del centro',templates:'Plantillas',procedures:'Procedimientos',providers:'Proveedores',pending:'Agenda de trabajo',clients:'Gestiones con clientes',directory:'Directorio',improvements:'Compras y mejoras',analysis:'Análisis económico'};
...
function accessBrowserLabel(){const ua=navigator.userAgent||'';if(/Edg\//i.test(ua))return'Edge';if(/Chrome\//i.test(ua))return'Chrome';if(/Firefox\//i.test(ua))return'Firefox';if(/Safari\//i.test(ua))return'Safari';return'Navegador web'}
function accessDeviceLabel(){const platform=navigator.userAgentData?.platform||navigator.platform||'';if(/mac/i.test(platform))return'Ordenador Mac';if(/win/i.test(platform))return'Ordenador Windows';if(/linux/i.test(platform))return'Ordenador Linux';if(/iphone|ipad/i.test(navigator.userAgent||''))return'iPhone / iPad';if(/android/i.test(navigator.userAgent||''))return'Dispositivo Android';return'Dispositivo no identificado'}
function renderAccessLogs(){const body=$('access-table');if(!body)return;const rows=Array.isArray(appDocument.accessLogs)?[...appDocument.accessLogs].sort((a,b)=>String(b.at||'').localeCompare(String(a.at||''))):[];body.innerHTML=rows.length?rows.map(row=>`<tr><td>${htmlEscape(row.at?new Date(row.at).toLocaleString('es-ES',{dateStyle:'short',timeStyle:'short'}):'Sin fecha')}</td><td>${htmlEscape(row.account||'Sin indicar')}</td><td>${htmlEscape(row.device||'Sin identificar')}</td><td>${htmlEscape(row.browser||'Sin identificar')}</td></tr>`).join(''):'<tr><td colspan="4" class="empty">Todavía no hay accesos registrados.</td></tr>';$('access-count').textContent=`${rows.length} accesos`}
async function recordAccess(){if(!firebaseUser||!appRevision)return;try{if(sessionStorage.getItem('aroa_access_logged')==='1')return}catch(error){}const logs=Array.isArray(appDocument.accessLogs)?[...appDocument.accessLogs]:[];logs.push({id:crypto.randomUUID(),at:new Date().toISOString(),account:firebaseUser.email||FIREBASE_ALLOWED_EMAIL,device:accessDeviceLabel(),browser:accessBrowserLabel()});const payload=structuredClone(appDocument);payload.accessLogs=logs.slice(-100);payload.updated=new Date().toISOString();payload.lastMutation=crypto.randomUUID();try{const confirmed=await writeDocument('appState','main',payload,appRevision);applyConfirmedDocument(confirmed);try{sessionStorage.setItem('aroa_access_logged','1')}catch(error){}}catch(error){console.warn('No se pudo registrar el acceso',error)}}
...
function exportCurrentWorkbook(){
  ...
  const improvements=[['Concepto','Tipo','Fecha','Proveedor','N.º de proveedor SAP','N.º de solicitud','N.º de pedido','Importe sin IVA','Estado','Justificación','Actuación realizada','Resultado','Fecha de solicitud','Fecha de pedido','Fecha de finalización','Tipo de gasto','Actuación visible','Foto anterior','Foto posterior','Observaciones'],...improvementRows.map(r=>[r.name||'',r.category||'',r.date||'',r.provider||'',r.supplierNumber||'',r.requestNumber||r.request||'',r.orderNumber||'',formatMoneyValue(r.amount)||'',r.status||'Necesidad detectada',r.justification||'',r.action||'',r.result||'',r.requestDate||'',r.orderDate||'',r.completionDate||'',r.expenseType||'',r.visible?'Sí':'No',r.beforeUrl||'',r.afterUrl||'',r.observations||''])];
  ...
  const templates=[['Plantilla','Enlace'],...templateRows.map(r=>[r.name||'',r.url||''])];
  ...
  [
    ...
    ['Compras y mejoras',improvements],
...
function renderPending(){
  ...
  if($('pending-heading'))$('pending-heading').textContent=person==='all'?'Toda la agenda':`Agenda con ${person}`;
...
$('center-sort')?.remove();
 renderCenter();renderPending();renderClients();renderTemplates();renderProcedures();renderImprovements();
startFirebaseRest();
function init(){
 window.person='all';
 ...
 renderCenter();renderPending();renderClients();renderTemplates();renderProcedures();renderProviders();renderImprovements();renderDirectory();
}
...
let improvementEditorIndex=-1;
function improvementStatusClass(status){return {'Necesidad detectada':'open','Presupuesto solicitado':'process','Presupuesto recibido':'process','Solicitud creada':'process','Pendiente de aprobación':'process','Pedido emitido':'process','En ejecución':'process','Finalizado':'done','Descartado':'done'}[status]||'open'}
function improvementRequestLabel(row){const request=row.requestNumber||row.request||'',order=row.orderNumber||'';return [request,order].filter(Boolean).join(' · ')||'Sin tramitar'}
function improvementAmount(value){return formatMoneyValue(value)||'Sin indicar'}
function renderImprovements(){
 const query=String($('improvement-search')?.value||'').trim().toLocaleLowerCase('es'),category=$('improvement-category-filter')?.value||'all',filter=$('improvement-status-filter')?.value||'all',year=$('improvement-year-filter')?.value||'all',providerFilter=$('improvement-provider-filter')?.value||'all';
 const categories=[...new Set(improvementRows.map(row=>String(row.category||'').trim()).filter(Boolean))].sort((a,b)=>a.localeCompare(b,'es',{sensitivity:'base'}));
 const categorySelect=$('improvement-category-filter');if(categorySelect){const current=categorySelect.value;categorySelect.innerHTML='<option value="all">Todas las categorías</option>'+categories.map(item=>`<option value="${htmlEscape(item)}">${htmlEscape(item)}</option>`).join('');categorySelect.value=categories.includes(current)?current:'all'}
 const years=[...new Set(improvementRows.map(row=>String(row.orderDate||row.date||'').match(/(?:^|\/)(\d{4})$/)?.[1]).filter(Boolean))].sort((a,b)=>Number(b)-Number(a));
 const yearSelect=$('improvement-year-filter');if(yearSelect){const current=yearSelect.value;yearSelect.innerHTML='<option value="all">Todos los años</option>'+years.map(item=>`<option value="${item}">${item}</option>`).join('');yearSelect.value=years.includes(current)?current:'all'}
 const providers=[...new Set(improvementRows.map(row=>canonicalProvider(row.provider)).filter(Boolean))].sort((a,b)=>a.localeCompare(b,'es',{sensitivity:'base'}));const providerSelect=$('improvement-provider-filter');if(providerSelect){const current=providerSelect.value;providerSelect.innerHTML='<option value="all">Todos los proveedores</option>'+providers.map(item=>`<option value="${htmlEscape(item)}">${htmlEscape(item)}</option>`).join('');providerSelect.value=providers.includes(current)?current:'all'}
 const rows=improvementRows.filter(row=>(category==='all'||String(row.category||'').trim()===category)&&(filter==='all'||String(row.status||'Necesidad detectada')===filter)&&(year==='all'||String(row.orderDate||row.date||'').endsWith(`/${year}`))&&(providerFilter==='all'||canonicalProvider(row.provider)===canonicalProvider(providerFilter))&&[row.name,row.category,row.expenseType,row.orderDate,row.date,row.provider,row.supplierNumber,row.requestNumber,row.request,row.orderNumber,row.amount,row.status,row.justification,row.action,row.result,row.observations].join(' ').toLocaleLowerCase('es').includes(query)).sort((a,b)=>{const ao=String(a.orderNumber||'').trim(),bo=String(b.orderNumber||'').trim();if(!ao&&!bo)return 0;if(!ao)return 1;if(!bo)return -1;const an=Number(ao),bn=Number(bo);return Number.isFinite(an)&&Number.isFinite(bn)?an-bn:ao.localeCompare(bo,'es',{numeric:true,sensitivity:'base'})});
 const body=$('improvement-table');if(!body)return;
 body.innerHTML=rows.length?rows.map(row=>{const index=improvementRows.indexOf(row),supplier=String(row.supplierNumber||'').trim(),request=String(row.requestNumber||row.request||'').trim(),order=String(row.orderNumber||'').trim(),copyCell=(value,label)=>value?`<span class="copyable copyable-id" role="button" tabindex="0" data-copy-value="${htmlEscape(value)}" aria-label="Copiar ${label}" title="Copiar ${label}">${htmlEscape(value)}</span>`:'',photos=row.visible?[row.beforeUrl?`<a class="evidence-link evidence-present" href="${htmlEscape(row.beforeUrl)}" target="_blank" rel="noopener">Antes</a>`:'<span class="evidence-link evidence-missing" role="img" aria-label="Falta enlace de foto anterior">Antes</span>',row.afterUrl?`<a class="evidence-link evidence-present" href="${htmlEscape(row.afterUrl)}" target="_blank" rel="noopener">Después</a>`:'<span class="evidence-link evidence-missing" role="img" aria-label="Falta enlace de foto posterior">Después</span>'].join(' '):'',docs=[['PRES',row.budgetUrl],['ALB',row.deliveryNoteUrl],['FAC',row.invoiceUrl],['CERT',row.reportUrl]].map(([label,url])=>url?`<a class="evidence-link evidence-present" href="${htmlEscape(url)}" target="_blank" rel="noopener" title="Abrir ${label.toLowerCase()}">${label}</a>`:`<span class="evidence-link evidence-missing" role="img" aria-label="Falta enlace de ${label.toLowerCase()}" title="Falta enlace de ${label.toLowerCase()}">${label}</span>`).join(' ');return `<tr><td>${htmlEscape(row.name||'Sin indicar')}</td><td>${htmlEscape(row.category||'Sin indicar')}</td><td>${htmlEscape(row.expenseType||'Sin indicar')}</td><td class="date">${htmlEscape(row.orderDate||row.date||'Sin fecha')}</td><td class="copyable-cell">${copyCell(supplier,'el número de proveedor')}</td><td>${htmlEscape(canonicalProvider(row.provider)||'Sin indicar')}</td><td class="copyable-cell">${copyCell(request,'la solicitud')}</td><td class="copyable-cell">${copyCell(order,'el pedido')}</td><td>${htmlEscape(improvementAmount(row.amount))}</td><td><span class="status ${improvementStatusClass(row.status)}">${htmlEscape(row.status||'Necesidad detectada')}</span></td><td><button class="edit-btn" type="button" onclick="openImprovementEditor(${index})">Editar</button>${docs?`<div class="improvement-document-links">${docs}</div>`:''}${photos?`<div class="improvement-photo-links">${photos}</div>`:''}</td></tr>`}).join(''):'<tr><td colspan="11" class="empty">No hay registros que coincidan con los filtros.</td></tr>';
 body.querySelectorAll('[data-copy-value]').forEach(node=>{const copy=()=>copyImprovementValue(node.dataset.copyValue||'');node.addEventListener('click',copy);node.addEventListener('keydown',event=>{if(event.key==='Enter'||event.key===' '){event.preventDefault();copy()}})});
 $('improvement-count').textContent=`${rows.length} registros`;
 renderEconomicAnalysis();
}

function analysisAmount(value){return parseMoneyValue(value)||0}
function analysisDate(row){return String(row.orderDate||row.date||'').trim()}
function analysisYear(row){const match=analysisDate(row).match(/(\d{4})$/);return match?match[1]:'Sin año'}
function analysisCurrency(amount){return new Intl.NumberFormat('es-ES',{style:'currency',currency:'EUR',maximumFractionDigits:2}).format(amount||0)}
function renderAnalysisBars(nodeId,entries,total,emptyLabel){
 const node=$(nodeId);if(!node)return;
 if(!entries.length){node.innerHTML=`<p class="empty">${emptyLabel}</p>`;return}
 node.innerHTML=entries.slice(0,8).map(([label,value])=>{const percent=total?Math.max(3,(value/total)*100):0;return `<div class="analysis-bar-row"><div class="analysis-bar-label"><span>${htmlEscape(label)}</span><strong>${analysisCurrency(value)}</strong></div><div class="analysis-bar-track"><span class="analysis-bar-fill" style="width:${Math.min(100,percent)}%"></span></div></div>`}).join('');
}
function renderProviderAnalysisBars(entries,total,rows){
 const node=$('analysis-provider-chart');if(!node)return;
 if(!entries.length){node.innerHTML='<p class="empty">No hay proveedores para este filtro.</p>';return}
 const counts=new Map();rows.forEach(row=>{const label=String(row.provider||'Sin indicar').trim()||'Sin indicar';counts.set(label,(counts.get(label)||0)+1)});
 node.innerHTML=entries.slice(0,8).map(([label,value])=>{const percent=total?Math.max(3,(value/total)*100):0;const count=counts.get(label)||0;return `<div class="analysis-bar-row"><div class="analysis-bar-label"><button type="button" class="analysis-provider-link" data-provider-filter="${htmlEscape(label)}">${htmlEscape(label)}</button><strong>${analysisCurrency(value)} · ${count} registros</strong></div><div class="analysis-bar-track"><span class="analysis-bar-fill" style="width:${Math.min(100,percent)}%"></span></div></div>`}).join('');
 node.querySelectorAll('[data-provider-filter]').forEach(button=>button.addEventListener('click',()=>{const filter=$('improvement-provider-filter');if(filter){filter.value=button.dataset.providerFilter||'all';document.querySelector('[data-view="improvements"]')?.click();renderImprovements()}}));
}
function renderEconomicAnalysis(){
 const yearSelect=$('analysis-year-filter'),categorySelect=$('analysis-category-filter'),statusSelect=$('analysis-status-filter'),expenseSelect=$('analysis-expense-filter');
 if(!yearSelect||!categorySelect||!statusSelect||!expenseSelect)return;
 const years=[...new Set(improvementRows.map(analysisYear).filter(year=>year!=='Sin año'))].sort((a,b)=>Number(b)-Number(a));
 const categories=[...new Set(improvementRows.map(row=>String(row.category||'Sin categoría').trim()||'Sin categoría'))].sort((a,b)=>a.localeCompare(b,'es',{sensitivity:'base'}));
 const statuses=[...new Set(improvementRows.map(row=>String(row.status||'Necesidad detectada').trim()||'Necesidad detectada'))].sort((a,b)=>a.localeCompare(b,'es',{sensitivity:'base'}));
 const selected={year:yearSelect.value,category:categorySelect.value,status:statusSelect.value,expense:expenseSelect.value};
 yearSelect.innerHTML='<option value="all">Todos los años</option>'+years.map(year=>`<option value="${year}">${year}</option>`).join('');yearSelect.value=years.includes(selected.year)?selected.year:'all';
 categorySelect.innerHTML='<option value="all">Todas las categorías</option>'+categories.map(item=>`<option value="${htmlEscape(item)}">${htmlEscape(item)}</option>`).join('');categorySelect.value=categories.includes(selected.category)?selected.category:'all';
 statusSelect.innerHTML='<option value="all">Todos los estados</option>'+statuses.map(item=>`<option value="${htmlEscape(item)}">${htmlEscape(item)}</option>`).join('');statusSelect.value=statuses.includes(selected.status)?selected.status:'all';
 const filtered=improvementRows.filter(row=>{const rowYear=analysisYear(row),category=String(row.category||'Sin categoría').trim()||'Sin categoría',status=String(row.status||'Necesidad detectada').trim()||'Necesidad detectada',expense=String(row.expenseType||'Sin indicar').trim()||'Sin indicar';return (yearSelect.value==='all'||rowYear===yearSelect.value)&&(categorySelect.value==='all'||category===categorySelect.value)&&(statusSelect.value==='all'||status===statusSelect.value)&&(expenseSelect.value==='all'||expense.toLocaleLowerCase('es')===expenseSelect.value.toLocaleLowerCase('es'))});
 const total=filtered.reduce((sum,row)=>sum+analysisAmount(row.amount),0),fixed=filtered.filter(row=>String(row.expenseType||'').toLocaleLowerCase('es')==='fijo').reduce((sum,row)=>sum+analysisAmount(row.amount),0),extra=filtered.filter(row=>String(row.expenseType||'').toLocaleLowerCase('es')==='extra').reduce((sum,row)=>sum+analysisAmount(row.amount),0),finished=filtered.filter(row=>String(row.status||'').toLocaleLowerCase('es')==='finalizado').length,pending=filtered.filter(row=>!['finalizado','descartado'].includes(String(row.status||'').toLocaleLowerCase('es'))).length;
 const metrics=$('analysis-metrics');if(metrics)metrics.innerHTML=`<div class="metric"><span class="metric-label">Total filtrado</span><strong class="metric-value">${analysisCurrency(total)}</strong><small class="metric-note">${filtered.length} registros</small></div><div class="metric"><span class="metric-label">Gasto fijo</span><strong class="metric-value">${analysisCurrency(fixed)}</strong><small class="metric-note">Tipo fijo</small></div><div class="metric"><span class="metric-label">Gasto extra</span><strong class="metric-value">${analysisCurrency(extra)}</strong><small class="metric-note">Tipo extra</small></div><div class="metric"><span class="metric-label">Pendientes</span><strong class="metric-value">${pending}</strong><small class="metric-note">${finished} finalizadas</small></div>`;
 const aggregate=(key)=>{const map=new Map();filtered.forEach(row=>{const label=String(row[key]||'Sin indicar').trim()||'Sin indicar';map.set(label,(map.get(label)||0)+analysisAmount(row.amount))});return [...map.entries()].sort((a,b)=>b[1]-a[1])};
 const categoryEntries=aggregate('category');
 const palette=['#2f8069','#e47752','#6c8fbd','#c9a34e','#7c6db2','#4d9aa5','#d17a9c','#7c8d91'];
 const categoryNode=$('analysis-category-chart');
 if(categoryNode){if(!categoryEntries.length)categoryNode.innerHTML='<p class="empty">No hay importes para este filtro.</p>';else{let cursor=0;const stops=categoryEntries.slice(0,8).map(([label,value],i)=>{const start=cursor;cursor+=total?(value/total)*360:0;return `${palette[i%palette.length]} ${start}deg ${cursor}deg`}).join(',');categoryNode.innerHTML=`<div class="analysis-donut-wrap"><div class="analysis-donut" style="background:conic-gradient(${stops})"><span>${analysisCurrency(total)}</span></div><div class="analysis-legend">${categoryEntries.slice(0,8).map(([label,value],i)=>`<div><i style="background:${palette[i%palette.length]}"></i><span>${htmlEscape(label)}</span><strong>${analysisCurrency(value)} · ${Math.round(total?(value/total)*100:0)}%</strong></div>`).join('')}</div></div>`}}
 const selectedAnalysisYear=yearSelect.value,periodSelect=$('analysis-period-mode'),periodMode=periodSelect?.value||'extra';if(periodSelect){periodSelect.disabled=selectedAnalysisYear==='all';periodSelect.closest('.analysis-period-control')?.classList.toggle('is-hidden',selectedAnalysisYear==='all')}
 ...
 renderProviderAnalysisBars(aggregate('provider'),total,filtered);
 const expenseEntries=['Fijo','Extra'].map(label=>[label,filtered.filter(row=>String(row.expenseType||'').toLocaleLowerCase('es')===label.toLocaleLowerCase('es')).reduce((sum,row)=>sum+analysisAmount(row.amount),0)]).filter(([,value])=>value>0);
 const expenseNode=$('analysis-expense-chart');
 if(expenseNode){if(!expenseEntries.length)expenseNode.innerHTML='<p class="empty">Clasifica los registros como fijo o extra para ver este gráfico.</p>';else{let cursor=0;const expenseTotal=expenseEntries.reduce((sum,[,value])=>sum+value,0),expensePalette=['#2f8069','#e47752'],stops=expenseEntries.map(([label,value],i)=>{const start=cursor;cursor+=(value/expenseTotal)*360;return `${expensePalette[i]} ${start}deg ${cursor}deg`}).join(',');expenseNode.innerHTML=`<div class="analysis-donut-wrap"><div class="analysis-donut" style="background:conic-gradient(${stops})"><span>${analysisCurrency(expenseTotal)}</span></div><div class="analysis-legend">${expenseEntries.map(([label,value],i)=>`<div><i style="background:${expensePalette[i]}"></i><span>${label}</span><strong>${Math.round((value/expenseTotal)*100)}%</strong></div>`).join('')}</div></div>`}}
 const top=$('analysis-top-table');if(top){const rows=[...filtered].sort((a,b)=>analysisAmount(b.amount)-analysisAmount(a.amount)).slice(0,10);top.innerHTML=rows.length?rows.map(row=>`<tr><td>${htmlEscape(row.name||'Sin indicar')}</td><td>${htmlEscape(row.category||'Sin indicar')}</td><td>${htmlEscape(row.provider||'Sin indicar')}</td><td>${htmlEscape(row.orderNumber||'Sin indicar')}</td><td>${htmlEscape(analysisDate(row)||'Sin fecha')}</td><td>${htmlEscape(improvementAmount(row.amount))}</td><td>${htmlEscape(row.expenseType||'Sin indicar')}</td><td><span class="status ${improvementStatusClass(row.status)}">${htmlEscape(row.status||'Necesidad detectada')}</span></td></tr>`).join(''):'<tr><td colspan="8" class="empty">No hay compras que coincidan con los filtros.</td></tr>'}
}
function providerContactMatchKey(value){return canonicalProvider(value).toLocaleLowerCase('es').normalize('NFD').replace(/[\u0300-\u036f]/g,'').replace(/[^a-z0-9]/g,'')}
function purchaseProviderContactGroup(provider){const raw=canonicalProvider(provider);if(!raw)return'';const target=providerContactMatchKey(raw);const key=Object.keys(providerContactsByName||{}).find(name=>name!==INTERNAL_CONTACTS_KEY&&providerContactMatchKey(name)===target);return key||raw}
function purchaseProviderContacts(provider){const group=purchaseProviderContactGroup(provider);return group?(providerContactsByName[group]||[]):[]}
function purchaseProviderOptions(){const directory=providerDirectoryRows().map(row=>row.name);const values=[...directory,...(remoteKurroLists.provider||[]),...centerRows.map(r=>r.provider),...improvementRows.map(r=>r.provider)].map(canonicalProvider).filter(Boolean);return [...new Map(values.map(v=>[v.toLocaleLowerCase('es'),v])).values()].sort((a,b)=>a.localeCompare(b,'es'))}
function refreshImprovementProviderOptions(current=''){const select=$('improvement-edit-provider');if(!select)return;const value=current||select.value;select.innerHTML='<option value="">Sin proveedor seleccionado</option>'+purchaseProviderOptions().map(v=>`<option value="${htmlEscape(v)}">${htmlEscape(v)}</option>`).join('');if(value){const match=purchaseProviderOptions().find(v=>v.toLocaleLowerCase('es')===value.toLocaleLowerCase('es'));if(match)select.value=match;else{const option=document.createElement('option');option.value=value;option.textContent=value;select.append(option);select.value=value}}}
function renderImprovementContactDetails(){const details=$('improvement-edit-contact-details'),select=$('improvement-edit-contact'),contact=purchaseProviderContacts($('improvement-edit-provider')?.value).find(c=>c.id===select?.value);if(!details)return;if(!contact){details.innerHTML='<span class="provider-contact-empty">Elige un contacto del proveedor o completa los datos manualmente.</span>';return}details.innerHTML=[`<strong>${htmlEscape(contact.name||'Contacto')}</strong>`,contact.phone?`<a href="tel:${htmlEscape(contact.phone)}">${htmlEscape(contact.phone)}</a>`:'',contact.email?`<a href="mailto:${htmlEscape(contact.email)}">${htmlEscape(contact.email)}</a>`:''].filter(Boolean).join('<span> · </span>')||'<span class="provider-contact-empty">Sin datos de contacto guardados.</span>';if($('improvement-edit-contact-email'))$('improvement-edit-contact-email').value=contact.email||''}
function refreshImprovementContacts(selectedId=''){const select=$('improvement-edit-contact');if(!select)return;const contacts=purchaseProviderContacts($('improvement-edit-provider')?.value);select.innerHTML='<option value="">Sin contacto seleccionado</option>'+contacts.map(c=>`<option value="${htmlEscape(c.id)}">${htmlEscape(c.name)}</option>`).join('');if(selectedId&&contacts.some(c=>c.id===selectedId))select.value=selectedId;renderImprovementContactDetails()}
function improvementPhotoValues(row,kind){const list=kind==='before'?row.beforeUrls:row.afterUrls;const legacy=kind==='before'?row.beforeUrl:row.afterUrl;return Array.isArray(list)?list.filter(Boolean):legacy?[legacy]:[]}
function renderImprovementPhotoFields(kind,values=[]){const list=$(kind==='before'?'improvement-before-list':'improvement-after-list');if(!list)return;const urls=values.length?values:[''];list.innerHTML=urls.map((url,index)=>`<div class="improvement-photo-editor-row"><input type="url" inputmode="url" data-photo-input="${kind}" data-photo-index="${index}" value="${htmlEscape(url)}" placeholder="Pega el enlace de la foto ${kind==='before'?'anterior':'posterior'}"><button type="button" class="secondary improvement-photo-remove" data-photo-remove="${kind}" ${urls.length===1?'hidden':''}>Quitar</button></div>`).join('')}
function readImprovementPhotoValues(kind){return [...document.querySelectorAll(`[data-photo-input="${kind}"]`)].map(input=>input.value.trim()).filter(Boolean)}
function addImprovementPhotoField(kind){const values=readImprovementPhotoValues(kind);values.push('');renderImprovementPhotoFields(kind,values);$(kind==='before'?'improvement-before-list':'improvement-after-list')?.lastElementChild?.querySelector('input')?.focus()}
function syncImprovementEvidenceFields(){const visible=!!$('improvement-edit-visible')?.checked;['improvement-before-field','improvement-after-field'].forEach(id=>{const node=$(id);if(node)node.hidden=!visible})}
function openImprovementEditor(index=-1){improvementEditorIndex=index;const r=index<0?{}:improvementRows[index]||{};editorMessage($('improvement-editor'),'');$('improvement-edit-name').value=r.name||'';$('improvement-edit-category').value=r.category||'';$('improvement-edit-status').value=r.status||'Necesidad detectada';$('improvement-edit-date').value=dateForEditor(r.date||r.detectionDate);$('improvement-edit-request-date').value=dateForEditor(r.requestDate);$('improvement-edit-order-date').value=dateForEditor(r.orderDate);$('improvement-edit-completion-date').value=dateForEditor(r.completionDate);$('improvement-edit-justification').value=r.justification||'';$('improvement-edit-action').value=r.action||'';$('improvement-edit-result').value=r.result||'';refreshImprovementProviderOptions(r.provider||'');$('improvement-edit-provider').value=canonicalProvider(r.provider||'');$('improvement-edit-supplier').value=r.supplierNumber||'';$('improvement-edit-request').value=r.requestNumber||r.request||'';$('improvement-edit-order').value=r.orderNumber||'';$('improvement-edit-amount').value=formatMoneyValue(r.amount)||'';$('improvement-edit-expense').value=r.expenseType||'';$('improvement-edit-contact-email').value=r.contactEmail||'';$('improvement-edit-contact').value='';refreshImprovementContacts(r.contactId||'');$('improvement-edit-budget').value=r.budgetUrl||'';$('improvement-edit-delivery').value=r.deliveryNoteUrl||'';$('improvement-edit-invoice').value=r.invoiceUrl||'';$('improvement-edit-report').value=r.reportUrl||'';$('improvement-edit-visible').checked=!!r.visible;renderImprovementPhotoFields('before',improvementPhotoValues(r,'before'));renderImprovementPhotoFields('after',improvementPhotoValues(r,'after'));syncImprovementEvidenceFields();$('improvement-edit-observations').value=r.observations||'';$('delete-improvement-editor').hidden=index<0;$('improvement-editor-title').textContent=index<0?'Nueva compra o mejora':'Editar compra o mejora';$('improvement-editor').classList.add('open');$('improvement-editor').setAttribute('aria-hidden','false');$('improvement-edit-name').focus()}

function closeImprovementEditor(){if(!$('improvement-editor')||!allowEditorClose('improvement-editor'))return;$('improvement-editor').classList.remove('open');$('improvement-editor').setAttribute('aria-hidden','true');improvementEditorIndex=-1}
async function saveImprovementEditor(){const name=sentenceCaseIfAllCaps($('improvement-edit-name').value.trim());if(!name){editorMessage($('improvement-editor'),'Escribe el concepto de la compra o mejora.');return}const provider=$('improvement-edit-provider').value.trim(),contact=purchaseProviderContacts(provider).find(c=>c.id===$('improvement-edit-contact').value),changes={name,category:sentenceCaseIfAllCaps($('improvement-edit-category').value.trim()),status:$('improvement-edit-status').value,date:$('improvement-edit-date').value?dateFromEditor($('improvement-edit-date').value):'',detectionDate:$('improvement-edit-date').value?dateFromEditor($('improvement-edit-date').value):'',requestDate:$('improvement-edit-request-date').value?dateFromEditor($('improvement-edit-request-date').value):'',orderDate:$('improvement-edit-order-date').value?dateFromEditor($('improvement-edit-order-date').value):'',completionDate:$('improvement-edit-completion-date').value?dateFromEditor($('improvement-edit-completion-date').value):'',justification:$('improvement-edit-justification').value.trim(),action:$('improvement-edit-action').value.trim(),result:$('improvement-edit-result').value.trim(),provider:canonicalProvider(provider),supplierNumber:$('improvement-edit-supplier').value.trim(),requestNumber:$('improvement-edit-request').value.trim(),orderNumber:$('improvement-edit-order').value.trim(),amount:formatMoneyValue($('improvement-edit-amount').value)||'',expenseType:$('improvement-edit-expense').value,contactId:$('improvement-edit-contact').value||'',contactName:contact?.name||'',contactEmail:$('improvement-edit-contact-email').value.trim(),budgetUrl:$('improvement-edit-budget').value.trim(),deliveryNoteUrl:$('improvement-edit-delivery').value.trim(),invoiceUrl:$('improvement-edit-invoice').value.trim(),reportUrl:$('improvement-edit-report').value.trim(),visible:$('improvement-edit-visible').checked,beforeUrls:readImprovementPhotoValues('before'),afterUrls:readImprovementPhotoValues('after'),beforeUrl:readImprovementPhotoValues('before')[0]||'',afterUrl:readImprovementPhotoValues('after')[0]||'',observations:$('improvement-edit-observations').value.trim()};return commitEditor('improvements',improvementEditorIndex,changes,'improvement-editor',closeImprovementEditor)}

async function deleteImprovementEditor(){if(improvementEditorIndex>=0&&confirm('¿Quieres eliminar esta solicitud o pedido?'))return commitEditor('improvements',improvementEditorIndex,null,'improvement-editor',closeImprovementEditor,true)}
document.querySelector('[data-new-provider]')?.addEventListener('click',()=>openProviderEditor());document.querySelectorAll('[data-close-provider]').forEach(button=>button.addEventListener('click',closeProviderEditor));$('save-provider-editor')?.addEventListener('click',saveProviderEditor);$('provider-search')?.addEventListener('input',renderProviders);$('provider-clear-search')?.addEventListener('click',()=>{$('provider-search').value='';renderProviders()});$('provider-edit-name')?.addEventListener('input',renderProviderModalContacts);$('add-provider-modal-contact')?.addEventListener('click',()=>toggleProviderModalContactForm(true));$('cancel-provider-modal-contact')?.addEventListener('click',()=>toggleProviderModalContactForm(false));$('save-provider-modal-contact')?.addEventListener('click',saveProviderModalContact);document.addEventListener('click',event=>{const edit=event.target.closest('[data-provider-modal-contact-edit]');if(edit)toggleProviderModalContactForm(true,edit.dataset.providerModalContactEdit);const remove=event.target.closest('[data-provider-modal-contact-delete]');if(remove)deleteProviderModalContact(remove.dataset.providerModalContactDelete)});
document.querySelector('[data-new-improvement]')?.addEventListener('click',()=>openImprovementEditor());document.querySelector('#add-improvement-provider')?.addEventListener('click',()=>addKurroOption('provider','improvement-edit-provider','Escribe el nuevo proveedor'));document.querySelectorAll('.improvement-add-photo').forEach(button=>button.addEventListener('click',()=>addImprovementPhotoField(button.dataset.photoKind)));document.addEventListener('click',event=>{const button=event.target.closest('[data-photo-remove]');if(!button)return;const kind=button.dataset.photoRemove,values=readImprovementPhotoValues(kind);values.splice(Number(button.closest('.improvement-photo-editor-row')?.querySelector('input')?.dataset.photoIndex||0),1);renderImprovementPhotoFields(kind,values)});document.querySelector('[data-import-sap]')?.addEventListener('click',()=>$('improvement-sap-import')?.click());$('improvement-sap-import')?.addEventListener('change',event=>{const file=event.target.files?.[0];if(file)importSapWorkbook(file)});document.querySelectorAll('[data-close-improvement]').forEach(button=>button.addEventListener('click',closeImprovementEditor));$('save-improvement-editor')?.addEventListener('click',saveImprovementEditor);$('delete-improvement-editor')?.addEventListener('click',deleteImprovementEditor);$('improvement-search')?.addEventListener('input',renderImprovements);$('improvement-edit-visible')?.addEventListener('change',syncImprovementEvidenceFields);$('improvement-edit-provider')?.addEventListener('change',()=>{refreshImprovementContacts('');$('improvement-edit-contact-email').value=''});$('improvement-edit-contact')?.addEventListener('change',renderImprovementContactDetails);$('improvement-category-filter')?.addEventListener('change',renderImprovements);$('improvement-status-filter')?.addEventListener('change',renderImprovements);$('improvement-year-filter')?.addEventListener('change',renderImprovements);$('improvement-provider-filter')?.addEventListener('change',renderImprovements);$('improvement-clear-search')?.addEventListener('click',()=>{$('improvement-search').value='';$('improvement-category-filter').value='all';$('improvement-status-filter').value='all';$('improvement-year-filter').value='all';$('improvement-provider-filter').value='all';renderImprovements()});
function providerDirectoryKey(name){return canonicalProvider(name).toLocaleLowerCase('es')}
function providerDirectoryRows(){const map=new Map();const add=(name)=>{const clean=canonicalProvider(name);if(!clean)return;const key=providerDirectoryKey(clean);if(!map.has(key))map.set(key,{name:clean});};(remoteKurroLists.provider||[]).forEach(add);centerRows.map(r=>r.provider).forEach(add);improvementRows.map(r=>r.provider).forEach(add);(providerDirectory||[]).forEach(row=>{const clean=canonicalProvider(row.name||'');if(!clean)return;const key=providerDirectoryKey(clean);map.set(key,{...map.get(key),...row,name:clean});});improvementRows.forEach(row=>{const key=providerDirectoryKey(row.provider);const item=map.get(key);if(item&&row.supplierNumber&&!item.supplierNumber)item.supplierNumber=String(row.supplierNumber).trim()});return [...map.values()].sort((a,b)=>a.name.localeCompare(b.name,'es',{sensitivity:'base'}))}
function renderProviders(){const body=$('provider-table');if(!body)return;const q=String($('provider-search')?.value||'').trim().toLocaleLowerCase('es');const rows=providerDirectoryRows().filter(row=>[row.name,row.supplierNumber,row.description].join(' ').toLocaleLowerCase('es').includes(q));body.innerHTML=rows.length?rows.map(row=>{const contacts=purchaseProviderContacts(row.name);return `<tr><td><span class="provider-directory-name">${htmlEscape(row.name)}</span></td><td>${row.supplierNumber?`<span class="copyable copyable-id" role="button" tabindex="0" data-copy-value="${htmlEscape(row.supplierNumber)}" title="Copiar número de proveedor">${htmlEscape(row.supplierNumber)}</span>`:'Sin indicar'}</td><td class="provider-directory-contacts">${contacts.length?`${contacts.length} contacto${contacts.length===1?'':'s'}`:'Sin contactos'}</td><td>${htmlEscape(row.description||providerDescription(row.name)||'Pendiente de clasificar')}</td><td><button type="button" class="edit-btn" onclick="openProviderEditor('${htmlEscape(providerDirectoryKey(row.name))}')">Editar</button></td></tr>`}).join(''):'<tr><td colspan="5" class="empty">No hay proveedores que coincidan con la búsqueda.</td></tr>';$('provider-count').textContent=`${rows.length} proveedores`}
let providerEditorKey='';
function providerModalContactGroup(){const name=canonicalProvider($('provider-edit-name')?.value||'');return name&&name!=='Ninguno'?(purchaseProviderContactGroup(name)||name):''}
function providerModalContacts(){const group=providerModalContactGroup();return group?(providerContactsByName[group]||[]):[]}
function providerEditorMessage(message=''){const el=$('provider-editor-message');if(el)el.textContent=message||''}
function renderProviderModalContacts(){const list=$('provider-editor-contacts-list'),summary=$('provider-editor-contact-summary');if(!list)return;const contacts=providerModalContacts();if(summary)summary.textContent=contacts.length?`${contacts.length} contacto${contacts.length===1?'':'s'} guardado${contacts.length===1?'':'s'}.`:'Sin contactos guardados.';list.innerHTML=contacts.length?contacts.map(contact=>`<div class="provider-modal-contact-row"><div class="provider-modal-contact-info"><strong>${htmlEscape(contact.name||'Contacto')}</strong><small>${htmlEscape([contact.phone,contact.email].filter(Boolean).join(' · ')||'Sin teléfono ni correo guardados')}</small></div><div class="provider-modal-contact-actions"><button type="button" class="secondary" data-provider-modal-contact-edit="${htmlEscape(contact.id)}">Editar</button><button type="button" class="danger-button" data-provider-modal-contact-delete="${htmlEscape(contact.id)}">Eliminar</button></div></div>`).join(''):'<p class="provider-contact-empty">Este proveedor todavía no tiene contactos.</p>'}
function toggleProviderModalContactForm(show,contactId=''){const form=$('provider-modal-contact-form');providerModalContactEditingId=show?contactId:'';if(form)form.hidden=!show;const contact=show?providerModalContacts().find(item=>item.id===contactId):null;if($('provider-modal-contact-name'))$('provider-modal-contact-name').value=contact?.name||'';if($('provider-modal-contact-phone'))$('provider-modal-contact-phone').value=contact?.phone||'';if($('provider-modal-contact-email'))$('provider-modal-contact-email').value=contact?.email||'';if($('save-provider-modal-contact'))$('save-provider-modal-contact').textContent=contact?'Guardar cambios':'Guardar contacto';if(show)$('provider-modal-contact-name')?.focus()}
async function saveProviderModalContact(){const group=providerModalContactGroup(),name=$('provider-modal-contact-name')?.value.trim(),phone=$('provider-modal-contact-phone')?.value.trim()||'',email=$('provider-modal-contact-email')?.value.trim()||'',editingId=providerModalContactEditingId;if(!group){providerEditorMessage('Escribe primero el nombre del proveedor.');return}if(!name){providerEditorMessage('Escribe el nombre del contacto.');return}if(!firebaseUser||!appRevision){providerEditorMessage('Actualiza los datos de Firebase antes de guardar.');return}const contacts=providerModalContacts(),key=contact=>[contact.name,contact.phone,contact.email].map(value=>String(value||'').trim().toLocaleLowerCase('es')).join('|'),existing=contacts.find(contact=>contact.id!==editingId&&key(contact)===key({name,phone,email}));if(existing){providerEditorMessage('Ese contacto ya está guardado para este proveedor.');return}const original=contacts.find(contact=>contact.id===editingId),contact=editingId&&original?{...original,name,phone,email}:{id:crypto.randomUUID(),name,phone,email};const payload=structuredClone(appDocument);payload.providerContacts=structuredClone(providerContactsByName);payload.providerContacts[group]=editingId?contacts.map(item=>item.id===editingId?contact:item):[...contacts,contact];payload.updated=new Date().toISOString();payload.lastMutation=crypto.randomUUID();try{const confirmed=await writeDocument('appState','main',payload,appRevision);applyConfirmedDocument(confirmed);providerModalContactEditingId='';toggleProviderModalContactForm(false);renderProviderModalContacts();renderProviders();providerEditorMessage(editingId?'Contacto actualizado.':'Contacto guardado.')}catch(error){providerEditorMessage(error.message==='CONFLICT'?'Los datos han cambiado. Pulsa Actualizar e inténtalo de nuevo.':'No se ha podido guardar el contacto. Comprueba la conexión y vuelve a intentarlo.')}}
async function deleteProviderModalContact(contactId){const group=providerModalContactGroup(),contacts=providerModalContacts();if(!group||!contacts.some(contact=>contact.id===contactId)||!confirm('¿Eliminar este contacto del proveedor?'))return;if(!firebaseUser||!appRevision){providerEditorMessage('Actualiza los datos de Firebase antes de guardar.');return}const payload=structuredClone(appDocument);payload.providerContacts=structuredClone(providerContactsByName);payload.providerContacts[group]=contacts.filter(contact=>contact.id!==contactId);payload.updated=new Date().toISOString();payload.lastMutation=crypto.randomUUID();try{const confirmed=await writeDocument('appState','main',payload,appRevision);applyConfirmedDocument(confirmed);toggleProviderModalContactForm(false);renderProviderModalContacts();renderProviders();providerEditorMessage('Contacto eliminado.')}catch(error){providerEditorMessage('No se ha podido eliminar el contacto. Comprueba la conexión y vuelve a intentarlo.')}}
function openProviderEditor(key=''){const row=providerDirectoryRows().find(item=>providerDirectoryKey(item.name)===key)||{};providerEditorKey=key;$('provider-edit-name').value=row.name||'';$('provider-edit-supplier').value=row.supplierNumber||'';$('provider-edit-description').value=row.description||'';$('provider-editor-title').textContent=key?'Editar proveedor':'Nuevo proveedor';providerEditorMessage('');toggleProviderModalContactForm(false);renderProviderModalContacts();$('provider-editor').classList.add('open');$('provider-editor').setAttribute('aria-hidden','false');$('provider-edit-name').focus()}
function closeProviderEditor(){const modal=$('provider-editor');if(!modal)return;modal.classList.remove('open');modal.setAttribute('aria-hidden','true');providerEditorKey=''}
async function saveProviderEditor(){const name=canonicalProvider($('provider-edit-name').value),oldRow=providerDirectoryRows().find(row=>providerEditorKey&&providerDirectoryKey(row.name)===providerEditorKey),oldName=oldRow?.name||'',oldGroup=canonicalProvider(oldName),newGroup=canonicalProvider(name);if(!name){showSyncToast('Escribe el nombre del proveedor');return}if(!firebaseUser||!appRevision){showSyncToast('Actualiza los datos de Firebase antes de guardar');return}const next=providerDirectoryRows().filter(row=>!providerEditorKey||providerDirectoryKey(row.name)!==providerEditorKey);next.push({name,supplierNumber:$('provider-edit-supplier').value.trim(),description:$('provider-edit-description').value.trim()});const payload=structuredClone(appDocument);payload.providerDirectory=next;payload.providerContacts=structuredClone(providerContactsByName);if(oldGroup&&newGroup&&oldGroup!==newGroup&&payload.providerContacts[oldGroup]){payload.providerContacts[newGroup]=[...(payload.providerContacts[newGroup]||[]),...payload.providerContacts[oldGroup].filter(oldContact=>!(payload.providerContacts[newGroup]||[]).some(newContact=>[newContact.name,newContact.phone,newContact.email].map(value=>String(value||'').toLocaleLowerCase('es')).join('|')===[oldContact.name,oldContact.phone,oldContact.email].map(value=>String(value||'').toLocaleLowerCase('es')).join('|')))];delete payload.providerContacts[oldGroup]}payload.lists=mergedKurroLists();payload.lists.provider=[...new Set([...(payload.lists.provider||[]),name])];payload.updated=new Date().toISOString();payload.lastMutation=crypto.randomUUID();try{const confirmed=await writeDocument('appState','main',payload,appRevision);applyConfirmedDocument(confirmed);closeProviderEditor();showSyncToast('Proveedor guardado')}catch(error){showSyncToast('No se ha podido guardar el proveedor')}}
...
function applyConfirmedDocument(doc){
 ...
 refreshKurroPeopleOptions();refreshClientOptions();refreshKURROMetrics();renderCenter();renderPending();renderClients();renderTemplates();renderProcedures();renderProviders();renderImprovements();renderEconomicAnalysis();renderAccessLogs();
...
function excelSerialDate(value){const n=Number(value);if(!Number.isFinite(n))return '';const d=new Date(Date.UTC(1899,11,30)+Math.round(n)*86400000);return `${String(d.getUTCDate()).padStart(2,'0')}/${String(d.getUTCMonth()+1).padStart(2,'0')}/${d.getUTCFullYear()}`}
async function mergeImportedImprovementRows(importRows){const sapByOrder=new Map(importRows.map(row=>[String(row.orderNumber||'').trim(),row]));const mergedRows=improvementRows.map(row=>{const order=String(row.orderNumber||row.request||'').trim(),sap=sapByOrder.get(order);if(!sap)return row;const imported=String(row.id||'').startsWith('sap');return {...row,provider:sap.provider,supplierNumber:sap.supplierNumber,orderNumber:sap.orderNumber,amount:sap.amount,date:sap.date,name:row.name||sap.name,category:row.category||sap.category,status:row.status||sap.status,observations:imported?sap.observations:(row.observations||sap.observations),source:imported?sap.source:row.source}});const existingOrders=new Set(mergedRows.map(row=>String(row.orderNumber||'').trim()).filter(Boolean));const nextImprovements=[...mergedRows,...importRows.filter(row=>!existingOrders.has(String(row.orderNumber||'').trim()))];if(JSON.stringify(nextImprovements)===JSON.stringify(improvementRows))return 0;const changeCount=nextImprovements.length-improvementRows.length;const payload=structuredClone(appDocument);payload.improvements=nextImprovements;payload.updated=new Date().toISOString();payload.lastMutation=crypto.randomUUID();const confirmed=await writeDocument('appState','main',payload,appRevision);applyConfirmedDocument(confirmed);return changeCount}
...
function normalizeImprovementOrderDates(){let changed=false;const rows=improvementRows.map(row=>{const order=String(row.orderNumber||'').trim(),documentDate=SAP_DOCUMENT_DATES[order];if(!documentDate)return row;const next={...row};if(next.date!==documentDate){next.date=documentDate;next.detectionDate=documentDate;changed=true}if(next.orderDate!==documentDate){next.orderDate=documentDate;changed=true}return next});return{rows,changed}}
async function loadRemoteData(){
 ...
 dataLoadInFlight=(async()=>{try{
  ...
  if(SAP_IMPORT_ROWS.length)await mergeImportedImprovementRows(SAP_IMPORT_ROWS);
  ...
  const contactsBefore=structuredClone(providerContactsByName);const normalized=moveEmbeddedPeopleToContacts(restored,providerContactsByName);normalized.center=normalized.center.map(row=>({...row,category:canonicalCategory(row.category),provider:canonicalProvider(row.provider)}));providerContactsByName=normalized.contacts;const contactsChanged=JSON.stringify(normalized.contacts)!==JSON.stringify(contactsBefore);const canonicalLists=structuredClone(remoteKurroLists);if(Array.isArray(canonicalLists.category))canonicalLists.category=[...new Set(canonicalLists.category.map(canonicalCategory).filter(Boolean))];
  const providers=normalizeProviderData();providers.center=normalized.center;
  if(JSON.stringify(normalized.center)!==JSON.stringify(centerRows)||contactsChanged||JSON.stringify(canonicalLists)!==JSON.stringify(remoteKurroLists)||providers.changed){const payload=structuredClone(appDocument);payload.center=providers.changed?providers.center:normalized.center;payload.improvements=providers.changed?providers.improvements:improvementRows;payload.lists=providers.changed?providers.lists:canonicalLists;payload.providerContacts=providers.changed?providers.contacts:normalized.contacts;payload.providerDirectory=providers.changed?providers.directory:providerDirectory;payload.updated=new Date().toISOString();payload.lastMutation=crypto.randomUUID();try{const confirmed=await writeDocument('appState','main',payload,appRevision);applyConfirmedDocument(confirmed)}catch(error){console.warn('No se pudieron normalizar los contactos y proveedores',error)}}
  return true;
...
async function commitEditor(kind,index,changes,modalId,close,remove=false){
 ...
 const rows={center:centerRows,pending:pendingRows,clients:clientRows,improvements:improvementRows}[kind];
